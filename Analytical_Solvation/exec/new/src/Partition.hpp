...
public:
	...
	dumpfile(dfile) {
		...
		this->ave_Nmm_a = -1;
		this->ave_Nms_a = -1;
		this->contact_constraint_constant = 2 * (this->Nm - 1) + (z-2) * (this->Nm-2) + (z-1) * 2;
		this->loc       = this->Nm - 1;
	...
	// methods
	int Nms_tot(int Nmm){
		return this->contact_constraint_constant - 2 * (Nmm);
	}
	...
	double p_chain(int Nmm) {
		...
		if (x <= 0 || x >= 1.0) {
			...
			beta_pdf = (this->alpha - 1.0) * std::log(x) +
							(this->beta - 1.0) * std::log1p(-x) -
							std::log(boost::math::beta(this->alpha, this->beta)) -
							std::log(scale);
	...
	void init_pw_power_cache(){
		...
		for(int i = 1; i <= max_n; i++) {
			this->pw_power_cache[i] = this->pw_power_cache[i-1] * (this->pw) < 0 ? big_float(0.0) : this->pw_power_cache[i-1] * (this->pw);
		}
		return;
	}
	...
	void init_ipw_power_cache(){
		...
		for(int i = 1; i <= max_n; i++) {
			this->ipw_power_cache[i] = this->ipw_power_cache[i-1] * (1-this->pw) < 0 ? big_float(0.0) : this->ipw_power_cache[i-1] * (1-this->pw);
		}
		return;
	}
	...
	big_float comb_mm(int Nmm, int Nmm_a) {
		big_float factorial_tot   = this->factorial_cache[Nmm]/(this->factorial_cache[Nmm_a] * this->factorial_cache[Nmm-Nmm_a]) * this->pw_power_cache[Nmm_a] * this->ipw_power_cache[Nmm-Nmm_a];
		return factorial_tot < 0 ? big_float(0.0) : factorial_tot;
	}

	// Function to calculate combinations (comb_mm)
	big_float comb_ms(int Nmm, int Nms_a) {
		int Nms = this->Nms_tot(Nmm);
		big_float factorial_tot   = this->factorial_cache[Nms]/(this->factorial_cache[Nms_a] * this->factorial_cache[Nms-Nms_a]) * this->pw_power_cache[Nms_a] * this->ipw_power_cache[Nms-Nms_a];
		return factorial_tot < 0 ? big_float(0.0) : factorial_tot;
	}

	// get the energy
	double Energy(int Nmm, int Nmm_a, int Nms_a) {
		double energy = Nmm_a * this->EMM_A + (Nmm - Nmm_a) * this->EMM_N + Nms_a * this->EMS_A + (this->Nms_tot(Nmm) - Nms_a) * this->EMS_N;
		return energy;
	}
	...
	big_float boltzmann(int Nmm, int Nmm_a, int Nms_a){
		...
		return boltz < 0 ? big_float(0.0) : boltz;
	}

	// run the for loop
	void get_partition_weights(){
		std::cout << "Instantiating weights..." << std::endl;
		double    S_chain = 0;
		big_float S_Nmm   = 0;
		big_float S_Nms   = 0;
		...
		std::cout << "About to jump into the loop." << std::endl;
		std::cout << "this->Nm = " << this->Nm << ", this->Nmm_max = " << this->Nmm_max << std::endl;

		for (int Nmm{this->Nm-1}; Nmm < this->Nmm_max+1; ++Nmm){
			if (Nmm%50==0){
			...
			for (int Nmm_a{0}; Nmm_a < Nmm+1; ++Nmm_a){
				...
				for (int Nms_a {0}; Nms_a < this->Nms_tot(Nmm)+1; ++Nms_a){
					...
					// std::cout << "energy = " << this->energy_store.back() << std::endl;
				}
			}
			if (Nmm%50==0){
	...
	void get_partition_weights_quick(){
		std::cout << "Instantiating weights (for quicker compute)..." << std::endl;
		double    S_chain = 0;
		big_float S_Nms   = 0;
		big_float boltzmann_factor = 0;
		big_float energy_factor    = 0;
		...
		std::cout << "About to jump into the loop for " << this->T << "." << std::endl;
		std::cout << "this->Nm = " << this->Nm << ", this->Nmm_max = " << this->Nmm_max << std::endl;
		...
		for (int Nmm{this->Nm-1}; Nmm < this->Nmm_max+1; ++Nmm){
			...
			S_chain = this->p_chain(Nmm);
			for (int Nms_a {0}; Nms_a < this->Nms_tot(Nmm)+1; ++Nms_a){
				// std::cout << "@ Nms_a = " << Nms_a << std::endl;
				S_Nms            = this->comb_ms(Nmm, Nms_a);
				energy_factor    = big_float(this->Energy(Nmm, 0, Nms_a));
				boltzmann_factor = S_chain * S_Nms * boost::multiprecision::exp(-1/this->T * energy_factor);
 				this->weights_store.push_back (boltzmann_factor);
				this->energy_store.push_back  (energy_factor * boltzmann_factor);
				this->energy2_store.push_back (energy_factor * energy_factor * boltzmann_factor);
				this->Nmm_store.push_back     (Nmm     * boltzmann_factor);
				this->Nmm_a_store.push_back   (0);
				this->Nms_a_store.push_back   (Nms_a   * boltzmann_factor);
	...
	void get_partition(){
		this->Z = std::accumulate(this->weights_store.begin(), this->weights_store.end(), big_float(0.0));
		return;
	}

	void get_free_energy(){
		this->free_energy = -this->T*boost::multiprecision::log(this->Z);
		return;
	}
