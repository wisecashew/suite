...
int main (int argc, char** argv) {
	...
	int option_index = 0;
	while ((opt = getopt_long(argc, argv, ":D:d:T:p:M:m:S:s:a:b:h", long_options, &option_index)) != -1){
		switch(opt) {
			case 'D':
				Nm = std::stoi(optarg);
				break;
	...
	Partition my_partition (Nm, 1.0, pw, E_MM_A, E_MM_N, E_MS_A, E_MS_N, alpha, beta, dumpfile);

	std::cout << "p_chain(0) = "   << my_partition.p_chain(0)   << std::endl;
	std::cout << "p_chain(31) = "  << my_partition.p_chain(31)  << std::endl;
	std::cout << "p_chain(100) = " << my_partition.p_chain(100) << std::endl;
	std::cout << "p_chain(208) = " << my_partition.p_chain(208) << std::endl;
	std::cout << "p_chain(250) = " << my_partition.p_chain(250) << std::endl;
	std::cout << "p_chain(300) = " << my_partition.p_chain(300) << std::endl;
