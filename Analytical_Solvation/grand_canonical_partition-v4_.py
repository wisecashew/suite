	...
	for T in T_range:
		...
		for xc_bulk in xcb:
			...
			if xc_bulk == 0:
				...
				omega_mix     = mp.factorial(Nsolvent)/(mp.factorial(Ns) * mp.factorial(Nc))
				energy        = lambda Nmm, Nms: emm * (Nmm) + ems * Nms + emc * (Nms_tot(Nmm) - Nms) + Ns * Nc / (Ns+Nc) * z * esc 
				boltzmann     = lambda Nmm, Nms: p_chain(Nmm) * mp.exp(-1/(kb * T) * (energy(Nmm, Nms)))
