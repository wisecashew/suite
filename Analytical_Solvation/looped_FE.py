...
if __name__=="__main__":
	...
	for idx, T in enumerate(T_list):
		...
		for xc_b in xc_bulk:
			f = lambda xs, xc: chi_ms * (1 - xs - xc) * xs + chi_mc * (1 - xs - xc) * xc + \
			(1 - xs - xc)/Nm * np.log(1 - xs - xc) + xs * np.log(xs) + xc * np.log(xc) - mu_s(xc_b) * xs - mu_c(xc_b) * xc

			# f = lambda xs, xc: chi_ms * Nm * xs + chi_mc * Nm * xc + \
			# + xs * np.log(xs) + xc * np.log(xc) - mu_s(xc_b) * xs/T - mu_c(xc_b) * xc/T
			...
			Ntot = Nm/(1-XS[idx_min]-XC[idx_min])
			Ns_solv.append(XS[idx_min])
			Nc_solv.append(XC[idx_min])
			Nm_solv.append(1-XS[idx_min]-XC[idx_min])
			N_solv.append(Ntot)
