...
if __name__=="__main__":
	...
	for idx, T in enumerate(T_list):
		...
		for xc_b in xc_bulk:
			F1      = kb * T * ((NTOT-Nm)*xc_b * np.log(xc_b) + (NTOT-Nm)*(1-xc) * np.log(1-xc_b) + (NTOT-Nm)*xc_b*(1-xc_b)*chi_sc)
			...
			delta_F       = lambda Nmm, x: kb * T * (NTOT-Nm-Nsolv(Nmm)) * ((1-x) * np.log(1-x) + x * np.log(x) + chi_sc * x * (1-x)) - F1
			...
			f             = lambda Nmm, x: energy(Nmm, x) - T * entropy_mix(Nmm, x) + delta_F
