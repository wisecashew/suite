...
int main(int argc, char** argv) {
    ...
    lattice_file_write {"__blank__"}, lattice_file_read {"__blank__"}, WL_file ("__blank__");  
    bool v = false, r = false;

    while ( (opt = getopt(argc, argv, ":W:s:L:R:f:M:o:u:p:t:e:vhr")) != -1 )
    {
        switch (opt) 
        {
            case 'f':
            ...
            case 'h':
                ...
                "Wang-Landau parameters    [-W]           (STRING ARGUMENT REQUIRED)     Name of Wang-Landau parameter file.\n" <<
    ...
    // ExtractParametersFromWL extracts all the Wang-Landau algorithm parameters
    std::array  <double,5> WL_vec {ExtractWLParamsFromFile (WL_file)}; 
    double Emin = WL_vec[0], Emax = WL_vec[1], NBins = static_cast<int>(WL_vec[2]), flatness = WL_vec[3], f = WL_vec[4]; 
    double deltaE = (Emax-Emin)/NBins; 
    std::vector <double> g (NBins, 1); g.reserve ( NBins ); 
    std::vector <int> Histogram (NBins, 0); Histogram.reserve ( NBins ); 
    ...
    std::cout << "Wang-Landau parameters: " << std::endl; 
    std::cout << "Emin = " << Emin << ", Emax = " << Emax << ", number of bins = " << NBins << ", flatness criterion = " << flatness << ", (scaling) f = " << f <<".\n\n";
    ...
    for (int i = step_number+1; i< (step_number+max_iter+1); i++) {
        ...
        if ( f < 1e-8 ){
            std::cout << "You have reached tolerance for \"f\". Breaking out... " << std::endl;
            break;
        }
        ...
        if ( IMP_BOOL ) {
            ...
            E1_idx = std::floor ( (sysEnergy-Emin )/deltaE) ;
            E2_idx = std::floor ( (sysEnergy_-Emin)/deltaE) ;
            ...
            if ( WangLandauAcceptance ( g[E1_idx], g[E2_idx] ) ){


                g[E2_idx] = g[E2_idx]*f; 
                Histogram[E2_idx] += 1; 

                aveH = std::accumulate (Histogram.begin(), Histogram.end(), 0.0)/NBins; 
                minH = *std::min_element (Histogram.begin(), Histogram.end());

                if ( minH > aveH*0.8 ){
                    std::cout << "Histogram flat. Updating scaler f..." << std::endl;
    ...
    dumpHistograms          (Histogram, g, Emin, Emax, NBins); 
