...
void FHP::perturb_particle_swap_with_monomer(std::array<int,3> monomer_location, int polymer_idx, int monomer_idx, int other_particle_idx){
...
void FHP::perturb_particle_swap_with_monomer_update(std::array<int,3> monomer_location, int polymer_idx, int monomer_idx, int other_particle_idx){
	...
	this->neighbor_pair_interaction(this->Polymers[polymer_idx].chain[monomer_idx], this->Lattice[other_particle_idx], this->rotation_container.cpair_initial, this->rotation_container.Epair_initial);
	...
	this->neighbor_pair_interaction(this->Polymers[polymer_idx].chain[monomer_idx], this->Lattice[other_particle_idx], this->rotation_container.cpair_final, this->rotation_container.Epair_final);

	return;
}

void FHP::perturb_particle_swap(Particle* tmp_par_ptr, int lat_idx_1, int lat_idx_2){
...
void FHP::perturb_particle_swap_with_update(Particle* tmp_par_ptr, int lat_idx_1, int lat_idx_2){
	...
	this->neighbor_pair_interaction(this->Lattice[lat_idx_1], this->Lattice[lat_idx_2], this->rotation_container.cpair_initial, this->rotation_container.Epair_initial);
	...
	this->neighbor_pair_interaction(this->Lattice[lat_idx_1], this->Lattice[lat_idx_2], this->rotation_container.cpair_final, this->rotation_container.Epair_final);
...
void FHP::perturb_orientation_sampler_forwards(std::array<double,CONTACT_SIZE_FHP>& contacts_sys, double E_sys, int iterator_idx, int lat_idx){
...
void FHP::perturb_choose_state_forward(int iterator_idx, int lat_idx){
...
void FHP::perturb_orientation_sampler_backwards_0(std::array<double,CONTACT_SIZE_FHP>& contacts_sys, double E_sys, int iteration_idx, int lat_idx){
...
void FHP::perturb_orientation_sampler_backwards(std::array<double,CONTACT_SIZE_FHP>& contacts_sys, double E_sys, int iteration_idx, int lat_idx){
...
void FHP::perturb_tail_rotation(int p_idx){

	// add attempt
	this->attempts[this->move_number] += 1;
	...
	std::array <double,CONTACT_SIZE_FHP> final_contacts = {0.0};
...
void FHP::perturb_head_rotation(int p_idx){
...
void FHP::revert_with_tail_biting(std::array<int,3>& to_slither, int p_idx){
...
void FHP::revert_without_tail_biting(Particle* tmp, std::array <int,3>& to_slither, std::array <int,3>& loc_0, int p_idx){
...
void FHP::revert_with_head_butting(std::array <int,3>& to_slither, int p_idx){
...
void FHP::revert_without_head_butting(Particle* tmp, std::array <int,3>& to_slither, std::array <int,3>& locf, int p_idx){
...
void FHP::forward_reptation_with_tail_biting(std::array<double,CONTACT_SIZE_FHP>& contacts_i, double& E_i, int p_idx){
	...
	std::array <double,CONTACT_SIZE_FHP> cm1_i     = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cm2_i     = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cpair_i   = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cm1_f     = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cm2_f     = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cpair_f   = {0.0};
	std::array <int,3>    loc_m1    = {0,0,0};
	std::array <int,3>    loc_m2    = {0,0,0};
	std::array <double,CONTACT_SIZE_FHP> contacts  = contacts_i;
	...
	for (int idx{0}; idx<deg_poly-1; ++idx) {
		...
		this->neighbor_pair_interaction(this->Polymers[p_idx].chain[idx], this->Polymers[p_idx].chain[idx+1], cpair_i, Epair_i);
		...
		this->neighbor_pair_interaction(this->Polymers[p_idx].chain[idx], this->Polymers[p_idx].chain[idx+1], cpair_f, Epair_f);
...
void FHP::forward_reptation_without_tail_biting(std::array<double,CONTACT_SIZE_FHP>& contacts_i, std::array<int,3>& to_slither, double& E_i, int p_idx){
	...
	std::array <double,CONTACT_SIZE_FHP> cm_i     = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cs_i     = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cpair_i  = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cm_f     = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cs_f     = {0.0};
	std::array <double,CONTACT_SIZE_FHP> cpair_f  = {0.0};
	...
	for (int idx{0}; idx<deg_poly; ++idx){
		...
		this->neighbor_pair_interaction(this->Lattice[lattice_index(loc_m, this->y, this->z)], this->Lattice[lattice_index(loc_s, this->y, this->z)], cpair_i, Epair_i);
		...
		this->neighbor_pair_interaction(this->Lattice[lattice_index(loc_s, this->y, this->z)], this->Lattice[lattice_index(loc_m, this->y, this->z)], cpair_f, Epair_f);
...
void FHP::backward_reptation_with_head_butting(std::array<double,CONTACT_SIZE_FHP>& contacts_i, double& E_i, int p_idx){
	...
	std::array <double,CONTACT_SIZE_FHP> cm1_i     = {0};
	std::array <double,CONTACT_SIZE_FHP> cm2_i     = {0};
	std::array <double,CONTACT_SIZE_FHP> cpair_i   = {0};
	std::array <double,CONTACT_SIZE_FHP> cm1_f     = {0};
	std::array <double,CONTACT_SIZE_FHP> cm2_f     = {0};
	std::array <double,CONTACT_SIZE_FHP> cpair_f   = {0};
	std::array <int,3>    loc_m1    = {0,0,0}; 
	std::array <int,3>    loc_m2    = {0,0,0}; 
	std::array <double,CONTACT_SIZE_FHP> contacts  = contacts_i;
	...
	for (int i{0}; i<deg_poly-1; ++i){
		...
		this->neighbor_pair_interaction(this->Lattice[lattice_index(loc_m1, this->y, this->z)], this->Lattice[lattice_index(loc_m2, this->y, this->z)], cpair_i, Epair_i);
		...
		this->neighbor_energetics(lattice_index(loc_m1, this->y, this->z), cm1_f, Em1_f);
		this->neighbor_energetics(lattice_index(loc_m2, this->y, this->z), cm2_f, Em2_f);
		this->neighbor_pair_interaction(this->Lattice[lattice_index(loc_m1, this->y, this->z)], this->Lattice[lattice_index(loc_m2, this->y, this->z)], cpair_f, Epair_f);
...
void FHP::backward_reptation_without_head_butting(std::array<double,CONTACT_SIZE_FHP>& contacts_i, std::array<int,3>& to_slither, double& E_i, int p_idx){
	...
	std::array <double,CONTACT_SIZE_FHP> cm_i     = {0};
	std::array <double,CONTACT_SIZE_FHP> cs_i     = {0};
	std::array <double,CONTACT_SIZE_FHP> cpair_i  = {0};
	std::array <double,CONTACT_SIZE_FHP> cm_f     = {0};
	std::array <double,CONTACT_SIZE_FHP> cs_f     = {0};
	std::array <double,CONTACT_SIZE_FHP> cpair_f  = {0};
	std::array <int,3>    loc_m    = {0,0,0};
	std::array <int,3>    loc_s    = to_slither;
	std::array <double,CONTACT_SIZE_FHP> contacts = contacts_i;
	...
	for (int i{0}; i<deg_poly; ++i){
		...
		this->neighbor_energetics(lattice_index(loc_m, this->y, this->z), cm_f, Em_f);
		this->neighbor_energetics(lattice_index(loc_s, this->y, this->z), cs_f, Es_f);
		this->neighbor_pair_interaction(this->Lattice[lattice_index(loc_m, this->y, this->z)], this->Lattice[lattice_index(loc_s, this->y, this->z)], cpair_f, Epair_f);
...
void FHP::perturb_reptation_forward(int p_idx){
...
void FHP::perturb_reptation_backward(int p_idx){
...
void FHP::perturb_polymer_orientation_flip(int p_idx){
	...
	std::array<double,CONTACT_SIZE_FHP> contacts_post = {0.0}; // the contacts of the system after final perturbation
	...
	if ( rng_acc < std::exp (-1/this->T * (E_post - this->Energy)) * this->enhanced_flipper.prob_n_to_o/this->enhanced_flipper.prob_o_to_n){
		...
		//	std::cout << "E_post = " << E_post << ", sysEnergy = " << this->Energy << std::endl;
		// }
...
void FHP::perturb_solvation_shell_flip(){
...
void FHP::perturb_solvent_exchange_from_shell(){

	// set up the set of indicies
	std::set    <int> solvation_shell_set = this->get_solvation_shell();
	std::vector <int> solvation_shell_indices(solvation_shell_set.begin(), solvation_shell_set.end());
	...
	double rng_acc     = 0;
	std::array <double,CONTACT_SIZE_FHP> post_contacts = {0.0};
...
void FHP::perturb_solvent_exchange(){
...
void FHP::perturb_regrowth(int p_idx){
	...
	double rng_acc      =  0;
	std::array <double,CONTACT_SIZE_FHP> forw_contacts = {0.0};
...
void FHP::perturb_forward_head_regrowth(int p_idx, int m_idx){
		...
		while(idx_counter < this->enhanced_swing.ntest){
			...
			if (ne_list[idx_counter] == loc_m){
				...
				if (self_swap_idx < m_idx){
					...
					this->enhanced_swing.contacts_store[idx_counter].fill(-1);
					block_counter                                   += 1;
				}
				else {
...
void FHP::perturb_accept_after_head_regrowth(bool not_trap_bool){
...
void FHP::perturb_backward_head_regrowth(int p_idx, int m_idx, int recursion_depth){
...
void FHP::perturb_forward_tail_regrowth(int p_idx, int m_idx){
...
void FHP::perturb_accept_after_tail_regrowth(bool not_trap_bool){
...
void FHP::perturb_backward_tail_regrowth(int p_idx, int m_idx, int recursion_depth){
...
void FHP::perturb_system_simple(){
	this->move_number = rng_uniform(0, 2);
	switch (this->move_number) {
		case 0:
			if (rng_uniform(0,1) == 0){
...
void FHP::perturb_system_dry(){
	this->move_number = rng_uniform(0, 3);
...
void FHP::perturb_system_isotropic(){

	this->move_number = rng_uniform(0, 4);
	switch (this->move_number) {
		case 0:
			if (rng_uniform(0,1) == 0){
				this->perturb_tail_rotation(0);
...
void FHP::perturb_system_straight(){

	this->move_number = rng_uniform(0, 7);
	switch (this->move_number) {
		case 0:
			if (rng_uniform(0,1) == 0){
				this->perturb_tail_rotation(0);
