...
void ChainRegrowth_BIASED (std::vector <Polymer>* Polymers, std::vector <Polymer>* _Polymers, std::vector <Particle*>* Cosolvent, std::vector <Particle*>* LATTICE, \
	std::map <std::pair <std::string, std::string>, std::tuple <std::string, double, double, int, int>>* InteractionMap, \
	std::array <double,8>* contacts, bool* IMP_BOOL, \
	double* sysEnergy, double temperature, int* nhits, int p_index, int x, int y, int z){


	int deg_poly = (*Polymers)[p_index].deg_poly; 
	int m_index  = 1; // rng_uniform (1, deg_poly-2); 
	...
	if ( growth ){
		...
		if ( rng_acc < std::exp(-1/temperature * (frontflow_energy - *sysEnergy)) * prob_n_to_o/prob_o_to_n ){
			...
			reached = true;
			// std::cout << "Accepted with energy " << frontflow_energy << "." << std::endl;
			for (int i{m_index+1}; i < deg_poly; ++i){
				// std::cout << "new_cut = "; print(new_cut);
				
				if (new_cut[i-(m_index+1)] != (*_Polymers)[0].chain[i]->coords){
					reached = false;
					break;
				}
			}
			...
			if (reached){
				// std::cout << "Accepted, and new_cut = "; print(new_cut);
				*nhits += 1;
			}
		...
		if ( rng_acc < std::exp(-1/temperature * (frontflow_energy - *sysEnergy)) * prob_n_to_o/prob_o_to_n ) {
			...
			for (int i{0}; i < m_index; ++i){
				...
				if (new_cut[i] != (*_Polymers)[0].chain[i]->coords){
					reached = false;
					break;
				}
			}
...
void PerturbSystem_BIASED (std::vector <Polymer>* Polymers, std::vector <Polymer>* _Polymers, std::vector <Particle*>* Cosolvent, std::vector <Particle*>* LATTICE,  \
	std::map <std::pair<std::string, std::string>, std::tuple <std::string, double, double, int, int>>* InteractionMap, \
	std::array <double,8>* contacts, std::array <int,9>* attempts, \
	bool* IMP_BOOL, bool v, double* sysEnergy, double temperature, \
	int* move_number, int* nhits, int x, int y, int z) {

	int index = 0; // rng_uniform (0, static_cast<int>((*Polymers).size()-1) ); 
	int r     = 2; // rng_uniform (0, 8); 
	...
	switch (r) {
		...
		case (2):
			...
			ChainRegrowth_BIASED (Polymers, _Polymers, Cosolvent, LATTICE, InteractionMap, contacts, IMP_BOOL, sysEnergy, temperature, nhits, index, x, y, z);
			*move_number    = r; 
			(*attempts)[r] += 1;			
			break; 
