...
int main (int argc, char** argv) {
    ...
    std::string fpositions         {"__blank__"}; // name of file with final coords of polymer 
	...
	while ( (opt = getopt(argc, argv, ":l:s:L:R:F:f:M:o:u:p:t:e:vhSAyr")) != -1 )
	{
	    switch (opt) 
	    {
		case 'f':
		...
		case 'h':
			...
			"Initial polymer coordinates              [-p]           (STRING ARGUMENT REQUIRED)     Name of input file with initial coordinates of polymer.\n" <<
            "Final polymer coordinates                [-F]           (STRING ARGUMENT REQUIRED)     Name of input file with final coordinates of polymer.\n" <<
    ...
    std::vector <Polymer> _Polymers = ExtractPolymersFromFile(fpositions, x, y, z);
    ...
    for (int i = step_number+1; i < (step_number+max_iter+1); ++i) {
        ...
        PerturbSystem_BIASED (&Polymers, &_Polymers, &Cosolvent, &LATTICE, &InteractionMap, &contacts, &attempts, &IMP_BOOL, v, &sysEnergy, T, &move_number, &nhits, x, y, z); 
    ...
    std::cout << "Number of hits is " << nhits << "." << std::endl;
