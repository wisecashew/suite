...
int main (int argc, char** argv) {
    ...
    std::map <std::pair<std::string, std::string>, std::tuple<std::string, double, double, int, int>>::iterator it; 
    for (it = InteractionMap.begin(); it != InteractionMap.end(); it++){
        std::cout << "key = (" << (it->first).first << ", " << (it->first).second <<"), value = " << "(" << std::get<0>(it->second) << ", " << std::get<1>(it->second) << ", " << std::get<2>(it->second) << ", " << std::get<3>(it->second) << ", " << std::get<4>(it->second) <<")" << std::endl;
    }
    // exit (EXIT_SUCCESS);
    ...
    // std::array <double,8> contacts_c = {0,0,0,0,0,0,0,0};
    // double sysEnergy_ = CalculateEnergyRevamped (&Polymers, &Cosolvent, &LATTICE, &InteractionMap, &contacts_c, x, y, z);
    // if ( sysEnergy != sysEnergy_){
    //    std::cout << "Problem with energy computation..." << std::endl;
    //    exit (EXIT_FAILURE);
    // }
    // if ( contacts_c != contacts ){
    //    std::cout << "Contacts dont match..." << std::endl;
    //    exit (EXIT_FAILURE);
    // }
    ...
    for (int i = step_number+1; i < (step_number+max_iter+1); ++i) {
        ...
        PerturbSystem_BIASED_debug (&Polymers, &Cosolvent, &LATTICE, &InteractionMap, &E, &contacts, &attempts, &IMP_BOOL, v, &sysEnergy, T, &move_number, x, y, z); 
