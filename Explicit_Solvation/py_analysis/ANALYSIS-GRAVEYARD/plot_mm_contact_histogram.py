...
parser = argparse.ArgumentParser (description="Read the energydump, and plot the monomer-monomer contacts histogram") 
parser.add_argument ('-i', nargs='+', dest='i', type=str, action='store', help="Address to energy dump file to parse information.", default="energydump") 
...
if __name__ == "__main__": 
    ...
    for (legend, address, c) in zip(labels, args.i, colors):
        ...
        mm_store = df["mm_tot"].values 
        h = plt.hist (mm_store, bins=20, density=True, alpha=j, label=legend, ls='dashed', color=c, edgecolor='k')
        j = j/2 
        edict[k] = mm_store 
        k += 1
    ...
    plt.legend(labels)
    plt.savefig('mm_contacts_histogram_'+str(args.T)+'.png', dpi=1200) 
    plt.show() 

    print ("Moving on to calculate KL divergence...")

    # partition the mm-contacts store into a bunch of bins 
