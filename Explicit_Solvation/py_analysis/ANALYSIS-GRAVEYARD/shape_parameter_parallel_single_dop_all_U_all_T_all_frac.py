...
parser.add_argument('--coords', dest='e', action='store', type=str, metavar='coords', help='Name of coords file to parse through.') 
...
parser.add_argument('--png-name', dest='pn', action='store', help='name of imagefile.')
...
if __name__=="__main__":
    ...
    for U in U_list: 
        ...
        ax.errorbar ( temperatures, shape_mean, yerr=shape_std, fmt='o', markeredgecolor='k', \
                linestyle='-', elinewidth=1, capsize=0, linewidth=1, color=rgba_color, label='_nolegend_' ) 
        i+=1 
    ...
    if args.ev:
        shape_ev = get_shape_parameter_from_one_traj ("Uexcl", 8, 0.1, 1, args.e, 50000000)
        ax.errorbar ( temperatures, np.ones (len(temperatures))*shape_ev, yerr=0, linestyle='-')
