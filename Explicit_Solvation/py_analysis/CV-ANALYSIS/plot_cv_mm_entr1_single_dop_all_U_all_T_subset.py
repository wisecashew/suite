...
if __name__=="__main__":
    ...
    f = open ("CV-MM-output.mc", 'w')
    for U in U_list:
        rgba_color = cm.PiYG (divnorm (chi_list[i]) )
        plt.errorbar ( temperatures[-7:], PLOT_DICT[U][0][-7:]/args.dop, yerr=PLOT_DICT[U][1][-7:]/args.dop, linewidth=1, fmt='none', capsize=2, color='k', label="_nolabel_")
        plt.plot     ( temperatures[-7:], PLOT_DICT[U][0][-7:]/args.dop, linestyle='-', marker='o',  markeredgecolor='k', linewidth=3, color=rgba_color, label="_nolabel_", markersize=10)
        ...
        for j in range(len(PLOT_DICT[U][0])):
            f.write ("{:>2.2e} ".format(PLOT_DICT[U][0][j]/args.dop))
        f.write ("\n")
