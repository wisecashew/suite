...
if __name__=="__main__":
    ...
    ymin = 1
    ms_max = 1
    chi_list = [0.1, -0.1, -0.2] # [0.1, 0.05, 0.01, 0.001, 0, -0.001, -0.01, -0.1, -0.2]
    for U in U_list:
        rgba_color = cm.PiYG (divnorm (chi_list[i]) )
        dt = np.array(temperatures[1:]) - np.array(temperatures[:-1])
        dcov = np.gradient (PLOT_DICT[U][0], np.array(temperatures))
        plt.plot ( temperatures, dcov, linestyle='-', marker='o', markeredgecolor='k', linewidth=3, color=rgba_color, label="_nolabel_", markersize=10 )
