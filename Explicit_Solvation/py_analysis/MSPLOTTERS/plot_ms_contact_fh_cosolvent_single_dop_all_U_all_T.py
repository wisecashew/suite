...
if args.cs == 3:
    ...
    if args.ss == 'flory-huggins':
        divnorm = matplotlib.colors.SymLogNorm (0.001, vmin=-0.1, vmax=0.1 ) # this is for flory-huggins  
    elif args.ss == 'entropy':
        divnorm = matplotlib.colors.Normalize ( vmin=-0.2, vmax=0.1 ) # this is for entropy 
    else:
...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list: 
                ...
                ms_list = np.hstack ( (ms_list, df["ms1_tot"].values[-2000:] + df["ms2_tot"].values[-2000:] ) )
    ...
    ymin = 1
    for U in U_list:
        chi_1 = aux.get_chi_cosolvent ( str(U)+"/DOP_32/0.01/geom_and_esurf.txt" )[args.cs]
        print ("chi_1 = ", chi_1)
        if args.cs == 0: 
