...
if args.cs == 3:
    ...
    if args.ss == 'flory-huggins':
        divnorm = matplotlib.colors.SymLogNorm ( 0.05, vmin=-1000, vmax=1000 ) # this is for flory-huggins  
    elif args.ss == 'entropy':
        divnorm = matplotlib.colors.SymLogNorm ( 0.05, vmin=-1000, vmax=1000 ) # this is for entropy 
    else:
...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for u_idx in range(idx_range):
            if u_idx == idx_range-1:
                results = pool_list [ 0 ].starmap ( aux.obtain_order_parameter, zip( itertools.repeat(U), itertools.repeat(dop), master_temp_list[u_idx*nproc:], itertools.repeat(ortn_file), master_num_list[u_idx*nproc:], itertools.repeat(starting_index), itertools.repeat(args.norm) ) ) 
            else:
                results = pool_list [ 0 ].starmap ( aux.obtain_order_parameter, zip( itertools.repeat(U), itertools.repeat(dop), master_temp_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(ortn_file), master_num_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(starting_index), itertools.repeat(args.norm) ) ) 
    ...
    ##############################################################
    my_cmap = cm.PiYG
    sm = plt.cm.ScalarMappable ( cmap=my_cmap, norm=plt.Normalize(vmin=-1000, vmax=1000) )
    sm.set_clim (-1000, 1000)
    cbar = plt.colorbar(sm, orientation='vertical', boundaries=np.linspace(-1000,0,1000))
    cbar.set_ticks ( [-1000, -800, -400, -200, 0] )
    cbar.ax.tick_params (labelsize=14)
    cbar.set_ticklabels( [-1000, -100, -10, -1, 0 ] )
    ax.set_xscale('log')
