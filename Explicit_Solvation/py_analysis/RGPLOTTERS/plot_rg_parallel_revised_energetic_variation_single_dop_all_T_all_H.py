...
parser.add_argument('-H', metavar='H', dest='H', action='store', nargs='+', type=float, help='Enter enthalpies you want probed.')
...
def get_starting_ind ( U, enthalpy, num, dop, dumpfile):
    filename = U + "/DOP_" + str(dop) + "/E_" + str(enthalpy) + "/" + dumpfile + "_" + str(num) + ".mc"
...
def infiltrate_coords_get_rg ( U, enthalpy, num, dop, coords_files, starting_index ):

    filename = U + "/DOP_" + str(dop) + "/E_" + str(enthalpy) + "/"+ coords_files + "_" + str(num)+".mc" 
...
if __name__ == "__main__":    
    ...
    enthalpies.sort() 
    print ("H = ", args.H)
    divnorm = matplotlib.colors.Normalize (vmin = np.min(enthalpies), vmax=np.max(enthalpies))
    ######
    ...
    for H in enthalpies:
        ...
        for U in U_list:
            ...
            for num in num_list:
                master_index_list.append (get_starting_ind (U, H, num, dop, "energydump") )
