...
parser.add_argument('--excl_vol', dest='ev', type=int, action='store', help='flag to include excluded volume forcefield.', default=100)
...
if __name__ == "__main__":
    ...
    for U in U_list:
        ...
        ax.plot ( temperatures, v_list, linestyle='-.', marker='^', alpha=0.8 )
    
    ax.legend    (U_list, loc='best', fontsize=12)
    ax.set_ylabel("Flory exponent ($\\nu$)", fontsize=18) 
    ax.set_xlabel("Temperature", fontsize=18 )
    # ax.set_xticks(temperatures) 
    plt.savefig  ("flory_exponent_plots.png", dpi=1200)

    end = time.time()
    print("Time required in seconds is " + str(end-start)   )
    # plt.show() 
