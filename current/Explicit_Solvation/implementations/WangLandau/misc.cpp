...
std::array <double,5> ExtractWLParamsFromFile (std::string WL_file){

	std::array <double,5> WL_vec; 
	double info; 
	std::string mystring; 
	std::vector <std::string> contents = ExtractContentFromFile (WL_file); 

	std::regex Emin ("Emin"), Emax ("Emax"); 
	std::regex NBins ("NBins"), flatness ("Flatness"); 
	std::regex f ("f"), eof ("END OF FILE"); 

	for ( std::string& s: contents){

		if ( std::regex_search (s, Emin)){
			info = NumberExtractor (s); 
			WL_vec[0] = info; 
		}
		else if ( std::regex_search (s, Emax)){
			info = NumberExtractor (s); 
			WL_vec[1] = info; 
		}
		else if ( std::regex_search (s, NBins)){
			info = NumberExtractor (s); 
			WL_vec[2] = info; 
		}
		else if ( std::regex_search (s, flatness)){
			info = NumberExtractor (s); 
			WL_vec[3] = info; 
		}
		else if ( std::regex_search (s, f)) {
			info = NumberExtractor (s); 
			WL_vec[4] = info; 
		}
		else if ( std::regex_search (s, eof)) {
...
bool WangLandauAcceptance (double g1, double g2){
...
void InputParser(int dfreq, int max_iter, bool r,
	...
	std::string lattice_file_read, std::string WL_file){
	...
	if (!r) {
		...
		if ( positions== "__blank__" || topology == "__blank__" || dfile== "__blank__" || efile == "__blank__" || \
			mfile == "__blank__" || stats_file == "__blank__" || lattice_file_read == "__blank__" || WL_file == "__blank__" ) {
			std::cerr << "polymer coords file is " << positions <<",\ntopology is " << topology <<",\nWang-Landau parameter file is " << WL_file << ",\npolymer coordinate dump file is " << dfile << ",\nenergy dump file is " \
...
void dumpHistograms (std::vector <int> Histogram, std::vector <double> g, double Emin, double Emax, int NBins){

	std::ofstream hist_dump ( "histogram", std::ios::out ); 
	std::ofstream g_dump ( "g_dump", std::ios::out ); 
	double delta = (Emax-Emin)/NBins;
	for ( int i{1}; i<NBins+1; ++i ){
		hist_dump << "[" << Emin + (i-1)*delta << ", " << Emin + i*delta << "]" << "\t" << Histogram[i] << "\n"; 
	}

	for ( int i{1}; i<NBins; ++i ){
		g_dump << "[" << Emin + (i-1)*delta << ", " << Emin + i*delta << "]" << "\t" << g[i] << "\n"; 
	}
...
void dumpMoveStatistics (std::array <int,9>* attempts, std::array <int,9>* acceptances, int step, std::string stats_file){
    ...
    dump_file << "Single site orientation flips      - attempts: " << (*attempts)[7] <<", acceptances: " << (*acceptances)[7] << ", acceptance fraction: " << static_cast<double>((*acceptances)[7])/static_cast<double>((*attempts)[7]) << ".\n"; 
