...
int main (int argc, char** argv) {
	...
	while ( (opt = getopt(argc, argv, ":s:L:R:f:M:o:u:p:t:e:vhbyr")) != -1 )
	{
		...
		case 'h':
			...
			"This is a special case. I have the solvents only interacting in the antiparallel fashion! Meant only for single solvent system.\n" << 
    ...
    double bruteE = 0;
    std::array <double,8> bruteContacts = {0,0,0,0,0,0,0,0};
    ...
    for (int i = step_number+1; i < (step_number+max_iter+1); ++i) {
        ...
        if ( v ){
            ...
            // delete below later 
            std::cout << "Doing a brute force check..." << std::endl;
            bruteE = CalculateEnergy (&Polymers, &Cosolvent, &LATTICE, &E, &bruteContacts, x, y, z);
            if ( bruteE != sysEnergy || bruteContacts != contacts ) {
                std::cout << "Either energy or contacts is messed up in end rotation... " << std::endl; 
                std::cout << "bruteE = " << bruteE << ", sysEnergy = " << sysEnergy << ". " << std::endl; 
                std::cout << "bruteContacts = "; print (bruteContacts, ", "); std::cout << "contacts = "; print(contacts); 
                exit (EXIT_FAILURE); 
            }
            std::cout << "Energy and contacts look good!" << std::endl;
            // delete above later 
	}
