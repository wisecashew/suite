...
if __name__=="__main__":
	...
	PLOT_DICT [H] = (mma_mean/26, mmn_mean/26, ms1a_mean/26, ms1n_mean/26, ms2a_mean/26, ms2n_mean/26)
	print("done!", flush=True)
	j = 0


	ax.bar ( np.arange(len(frac_list)), PLOT_DICT[H][0], color ='darkred', width=0.8, edgecolor='k')
	ax.bar ( np.arange(len(frac_list)), PLOT_DICT[H][1], bottom = PLOT_DICT[H][0], color='lightcoral', width=0.8, edgecolor='k')
	ax.bar ( np.arange(len(frac_list)), PLOT_DICT[H][2], bottom = PLOT_DICT[H][0]+PLOT_DICT[H][1], color='steelblue', width=0.8, edgecolor='k')
	ax.bar ( np.arange(len(frac_list)), PLOT_DICT[H][3], bottom = PLOT_DICT[H][0]+PLOT_DICT[H][1]+PLOT_DICT[H][2], color='lightskyblue', width=0.8, edgecolor='k')
	ax.bar ( np.arange(len(frac_list)), PLOT_DICT[H][4], bottom = PLOT_DICT[H][0]+PLOT_DICT[H][1]+PLOT_DICT[H][2]+PLOT_DICT[H][3], color='darkgreen', width=0.8, edgecolor='k')
	# ax.bar ( np.arange(len(frac_list)), PLOT_DICT[H][5], bottom = PLOT_DICT[H][0]+PLOT_DICT[H][1]+PLOT_DICT[H][2]+PLOT_DICT[H][3]+PLOT_DICT[H][4], color='lightgreen', width=0.8, edgecolor='k')
	...
	for j in range(len(args.H)):
		...
		ax.set_xticklabels ([str(i) for i in frac_list], weight='bold')
