...
if __name__=="__main__":
	...
	for k in range( len(keys) ):
		for i in range(len(enthalpies)):
			...
			p_diff = (df_real_subset[keys[k]].values - df_id[keys[k]].values)/(df_id[keys[k]].values)
			ax[0][0].plot (1-df_id["x"].values, np.flip(p_diff), marker='o', linewidth=3/2, markersize=8/2, color=cm.summer(divnorm(enthalpies[i])), markeredgecolor='k')

		plt.savefig ("excess-plots-for-"+keys[k], bbox_inches='tight', dpi=1200)
		ax[0][0].cla()
