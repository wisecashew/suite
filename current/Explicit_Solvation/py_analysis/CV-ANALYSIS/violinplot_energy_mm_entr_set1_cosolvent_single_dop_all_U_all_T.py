...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list: 
                ...
                mm_list = np.hstack ( ( mm_list, f ) )
                g = df["ms1_aligned"].values[-2000:]*Ems1_a  + df["ms1_naligned"].values[-2000:]*Ems1_n
                ms_list = np.hstack ( ( ms_list, g) )
            mm_mean.append (mm_list) # = np.hstack ( (mm_mean, mm_list ) )
            ms_mean.append (ms_list)
    ...
    for U in U_list:
        ...
        # plt.errorbar ( temperatures, PLOT_DICT[U], yerr=PLOT_DICT[U][1], linewidth=1, fmt='none', capsize=2, color='k', label="_nolabel_")
        vplot1 = plt.violinplot     ( dataset=PLOT_DICT[U][0], positions=np.arange(len(temperatures)), showmeans=True, showmedians=True)
        vplot2 = plt.violinplot     ( dataset=PLOT_DICT[U][1], positions=np.arange(len(temperatures)), showmeans=True, showmedians=True)
