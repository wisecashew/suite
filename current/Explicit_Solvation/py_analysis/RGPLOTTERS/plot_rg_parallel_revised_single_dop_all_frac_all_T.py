...
if __name__ == "__main__":    
    ...
    print ("T = ", args.T)
    enthalpies = [float(elem) for elem in args.T] 
    enthalpies.sort() 
    ######
    ...
    for H in enthalpies:
        ...
        for u_idx in range (idx_range):
            if u_idx == idx_range-1:
                results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_rg, zip( master_U_list[u_idx*nproc:], itertools.repeat(H), master_num_list [u_idx*nproc:], itertools.repeat (dop), itertools.repeat (coords_files), itertools.repeat (starting_index) ) )
            else:
                results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_rg, zip( master_U_list[(u_idx)*nproc:(u_idx+1)*nproc], itertools.repeat(H), master_num_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(dop), itertools.repeat(coords_files), itertools.repeat(starting_index) ) )
    ...
    if len(temperatures) == 1:
        ...
        colors = cm.coolwarm (np.linspace(0,1,len(enthalpies)))
    ...
    for H in enthalpies:
        ...
        ax.plot     ( frac_list, PLOT_DICT[H][0]/rg_max, marker='o', markeredgecolor='k', linestyle='-', linewidth=3/2, c=colors[i], label='_nolegend_', markersize=4 ) 
        i += 1
    ...
    ax.set_yticks (np.arange(0.8, 1.3 , 0.1))
