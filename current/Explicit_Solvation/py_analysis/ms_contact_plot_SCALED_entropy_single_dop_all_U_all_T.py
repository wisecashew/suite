...
if __name__=="__main__":
    ...
    if args.ev:
        ...
        plt.errorbar ( temperatures, contacts/contacts, yerr = np.std( df["ms_tot"].values)/(np.sqrt(40)*contacts[0]), fmt='^', markeredgecolor='k', linestyle='-', elinewidth=1, capsize=0 ) 
    ...
    for U in U_list:
        ...
        plt.errorbar(temperatures, np.asarray(ms_mean)/contacts[0], yerr=np.asarray(ms_err)/contacts[0], fmt='o', markeredgecolor='k', linestyle='-', elinewidth=1, capsize=0, color=cm.copper(i/9), label='_nolegend_')   
        i += 1
    ...
    plt.savefig("DOP_"+str(args.dop)+"_SCALED_multiple_mscorr.png", dpi=800)
