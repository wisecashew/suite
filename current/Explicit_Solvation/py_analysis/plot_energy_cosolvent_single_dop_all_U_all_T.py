...
if args.cs == 3:
    ...
    divnorm = matplotlib.colors.SymLogNorm ( 0.002, vmin=-0.2, vmax=0.1 ) # this is for entropy 
...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list: 
                ...
                ms_list = np.hstack ( (ms_list, df["energy"] ) )
                
            # ms_err  = np.hstack ( (ms_err,  (np.std(ms_list)/np.sqrt(30) ) ) )
            ms_mean = np.hstack ( (ms_mean, np.mean(ms_list**2) - np.mean(ms_list)**2 ) )

        # if ms_max < np.max (ms_mean):
        #    ms_max = np.max (ms_mean)

        PLOT_DICT[U] = ( ms_mean, 0 ) 
        i += 1
    ...
    # ax.set_yticks (np.linspace (0.55,1,10))
    ax.minorticks_on() 
