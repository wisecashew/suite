...
if __name__=="__main__":
	...
	ylims   = [(-0.55,0.01),(-0.06,0.001), (-1.15, 0.2), (-1, 11), (-1.125, 0.375), (-1.125, 0.375)]
	ypads   = [3, 1, 1, 1, 1, 3]
	yticks  = [np.arange(-0.5, 0.1, 0.1), np.arange(-0.06, 0.001, 0.02), np.arange(-1.0, 0.375, 0.2), np.arange(0, 11, 2), np.arange(-1.0, 0.5, 0.25), np.arange (-1, 0.375, 0.25)]
	...
	for k in range( 4 ): # len(keys)):
		...
		for i in range(len(args.T)):
			...
			if k == 0 or k == 2:
				...
				ax[0][0].yaxis.set_major_formatter (matplotlib.ticker.StrMethodFormatter ('{x:1.0f}'))
			elif k == 1:
