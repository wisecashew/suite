...
if __name__=="__main__":
	...
	for line in f:
		if re.findall ("U\d+", line):
			U_re = re.findall ("U\d+", line)
			PLOT_DICT[U_re[0]] = {}
			continue
		elif re.findall ("flory exp:", line):
			flory_re = line.strip().split()[2:]
			continue
		elif re.findall ("T: ", line):
			T_re = line.strip().split()[1:]
			for i in range (len(T_re)):
				PLOT_DICT[U_re[0]][float(T_re[i])] = float(flory_re[i])
	...
	for T in temperatures:
		flory_list = [] 
		for U in U_list: 
			flory_list.append ( PLOT_DICT[U][T]/2 )
		plt.plot ( frac_list, flory_list, linestyle='-', marker='o',  markeredgecolor='k', linewidth=2, color=colors[i], label="_nolabel_", markersize=10)
		i += 1
