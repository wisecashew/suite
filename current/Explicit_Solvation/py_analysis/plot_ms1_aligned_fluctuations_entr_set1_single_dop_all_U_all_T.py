...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list: 
                ...
                # f = (df["ms1_aligned"].values[-3000:]  + 2*df["mm_aligned"].values[-3000:]) /(2*df["mm_tot"].values[-3000:]+df["ms1_tot"].values[-3000:]) # /df["ms1_tot"].values
                f = df["ms1_aligned"].values[-500:]/df["ms1_tot"].values[-500:] # +2*df["mm_aligned"][-1500:] # / (df["ms1_tot"].values[-1500:]+2*df["mm_tot"][-1500:]) )
                ms_list = np.hstack ( (ms_list, np.mean( f**2 )-(np.mean(f) )**2  ) )
            print (ms_list, flush=True)
    ...
    for U in U_list:
        ...
        plt.errorbar ( temperatures, PLOT_DICT[U][0] / (np.array(temperatures)**2), yerr=PLOT_DICT[U][1]/(np.array(temperatures)**2), linewidth=1, fmt='none', capsize=2, color='k', label="_nolabel_")
        plt.plot     ( temperatures, PLOT_DICT[U][0] / (np.array(temperatures)**2), linestyle='-', marker='o',  markeredgecolor='k', linewidth=3, color=rgba_color, label="_nolabel_", markersize=10)
