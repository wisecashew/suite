...
if __name__ == "__main__":    
    ...
    for T in temperatures:
        ...
        for u_idx in range (idx_range):
            if u_idx == idx_range-1:
                results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_rg, zip( master_U_list[u_idx*nproc:], itertools.repeat(T), master_num_list [u_idx*nproc:], itertools.repeat (dop), itertools.repeat (coords_files), itertools.repeat (starting_index) ) )
            else:
                results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_rg, zip( master_U_list[(u_idx)*nproc:(u_idx+1)*nproc], itertools.repeat(T), master_num_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(dop), itertools.repeat(coords_files), itertools.repeat(starting_index) ) )
    ...
    ax.set_yticks (np.arange(0.5, args.ymax+0.05, 0.5))
