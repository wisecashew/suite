...
parser.add_argument('-U', metavar='U', type=str, dest='U', action='store', help='pass an energy surface for which you would like to calculate shape factors.') 
...
def get_Rg_components (coords_arr, xlen, ylen, zlen): 
    ...
    coord_arr = unfuck_polymer(coords_arr, xlen, ylen, zlen) 
    r_com = np.mean(coord_arr, axis=0)
    N = coord_arr.shape[0]

    rsumx, rsumy, rsumz = 0, 0, 0
    for i in range(N):
        rsumx += np.linalg.norm ( coord_arr[i,0] - r_com[0] ) **2 
        rsumy += np.linalg.norm ( coord_arr[i,1] - r_com[1] ) **2
        rsumz += np.linalg.norm ( coord_arr[i,2] - r_com[2] ) **2 

    return [rsumx/N, rsumy/N, rsumz/N]
    ...
    for U in U_list:
        ...
        for T in temperatures:
            ...
            for key in master_dict: 
                components = get_Rg_components ( master_dict[key][0], dop+2, dop+2, dop+2 ) 
                delta_star = 1 - 3* ( ( (components[0]**2)*(components[1]**2)+(components[1]**2)*(components[2]**2)+(components[0]**2)*(components[2]**2) )/(components[0]**2 + components[1]**2 + components[2]**2)**2 )
                shape_param.append ( delta_star )

            shape_param_mean.append ( np.mean(shape_param[args.s:]) )         
            shape_param_std.append  ( np.std(shape_param[args.s:])/np.sqrt(len(shape_param[args.s::100]) ))
        
        shape_param_mean = np.array (shape_param_mean) 
        shape_param_std  = np.array (shape_param_std )    
       
        if U=="Uexcl":
            ax.errorbar   ( temperatures, shape_param_mean, yerr=shape_param_std, linewidth=3 )
        else:
            ax.errorbar   ( temperatures, shape_param_mean, yerr=shape_param_std, linewidth=1, color=cm.copper(i/9)) 
        i+=1 
    
    ax.legend     ( U_list, loc='best', fontsize=12)
    ax.set_xlabel ( "Temperature (reduced)", fontsize=18) 
    ax.set_ylabel ( "$\langle \delta ^* \\rangle$", fontsize=18) 
    ax.set_xticks ( np.arange(0, np.max(Tmax)+2,1) ) 
    plt.savefig   ( "DOP_"+str(args.dop)+"_shape_param.png", dpi=1200)
    plt.show() 
