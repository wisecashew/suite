...
int main(int argc, char** argv) {
    ...
    sysEnergy = CalculateEnergy(&PolymerVector, &SolventVector, x, y, z, Emm_a, Emm_n, Ems_a, Ems_n, Eb_f, Eb_u, &m_neighbors, &a_contacts, &n_contacts); 
    ...
    for (int i = step_number+1; i< (step_number+max_iter+1); i++) {
        ...
        if (IMP_BOOL){ 
            sysEnergy_ = CalculateEnergy (&PolymerVector_, &SolventVector_, x, y, z, Emm_a, Emm_n, Ems_a, Ems_n, Eb_f, Eb_u, &m_neighbors, &a_contacts, &n_contacts); 
