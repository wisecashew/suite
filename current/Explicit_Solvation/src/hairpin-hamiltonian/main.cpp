...
int main(int argc, char** argv) {
    ...
    const double Eb_f   = info_vec[8];
    const double Eb_u   = info_vec[9]; 
    const double E_kink = info_vec[10]; 
    std::cout << "Number of polymers is " << N << ".\n";
    std::cout << "x = " << x <<", y = " << y << ", z = "<< z << ", T = " << T << ".\n"; 
    std::cout << "Emm_a = " << Emm_a <<", Emm_n = " << Emm_n << ", Ems_a = "<< Ems_a << ", Ems_n = " << Ems_n << ", Eb_f = " << Eb_f <<", Eb_u = " << Eb_u << ".\n";  
    ...
    sysEnergy = CalculateEnergy(&PolymerVector, &SolventVector, x, y, z, Emm_a, Emm_n, Ems_a, Ems_n, Eb_f, Eb_u, E_kink, &m_neighbors, &a_contacts, &n_contacts); 
    ...
    for (int i = step_number+1; i< (step_number+max_iter+1); i++) {
        ...
        if (IMP_BOOL){ 
            sysEnergy_ = CalculateEnergy (&PolymerVector_, &SolventVector_, x, y, z, Emm_a, Emm_n, Ems_a, Ems_n, Eb_f, Eb_u, E_kink, &m_neighbors, &a_contacts, &n_contacts); 
