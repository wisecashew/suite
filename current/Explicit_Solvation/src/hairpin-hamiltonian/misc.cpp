...
std::array <double,11> ExtractTopologyFromFile(std::string filename){
    ...
    std::regex x ("x"), y ("y"), z ("z"), kT ("kT"), Emm_a ("Emm_a"), Emm_n ("Emm_n"), Ems_a ("Ems_a"), Ems_n ("Ems_n"), Eb_f ("Eb_f"), Eb_u ("Eb_u"), E_kink ("E_kink"), eof ("END OF FILE"); 
    //bool out_mat = true; 
    ...
    for (std::string s: contents){
    	...
    	else if (std::regex_search(s, E_kink)) {
    		double info = NumberExtractor (s); 
    		info_vec[10] = info; 
    	}
...
double CalculateEnergy(std::vector <Polymer>* Polymers, std::vector <Particle>* Solvent, int x, int y, int z, double Emm_a, double Emm_n, double Ems_a, double Ems_n, double Eb_f, double Eb_u, double E_kink, double* m_neighbor, int* a_contacts, int* n_contacts){
        ...
        for (const Particle& p: pmer.chain){
            ...
            for (const std::array <int, 3>& loc: ne_list){
                ...
                if (bonded){
            		...
            		Particle ptr {ParticleReporter (Polymers, Solvent, loc) };
    ...
    // 3-body energetic contribution 
    for ( Polymer& pmer: (*Polymers) ){

    	size_t nkinks = (pmer.findKinks()).size();
    	std::cout << "Number of kinks is " << nkinks << std::endl;
    	Energy += nkinks*E_kink;  
