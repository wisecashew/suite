...
std::array <double,11> ExtractTopologyFromFile(std::string filename){
    ...
    std::regex x ("x"), y ("y"), z ("z"), kT ("kT"), Emm_a ("Emm_a"), Emm_n ("Emm_n"), Ems_a ("Ems_a"), Ems_n ("Ems_n"), Ems_h ("Ems_h"), Eb_f ("Eb_f"), Eb_u ("Eb_u"), eof ("END OF FILE"); 
    ...
    for (std::string s: contents){
    	...
    	else if (std::regex_search (s, Ems_h)) {
            ...
            for (const std::array <int, 3>& loc: ne_list){
                ...
                for ( const Particle& p_: bonded_particles ){
                	...
                	if ( loc == p_.coords ) {
                		...
                		if ( p.orientation == p_.orientation ){
                			Energy += 0.5*Emm_a; 
                		}
                		else {
                			Energy += 0.5*Emm_n; 
...
void dumpEnergy (double sysEnergy, int step, double m_contacts, int a_contacts, int n_contacts, int h_contacts, std::string filename){
    ...
    dump_file << sysEnergy << " | " << m_contacts << " | " << a_contacts << " | " << n_contacts << " | " << h_contacts << " | " << step << "\n";
...
std::vector <Polymer> MoveChooser_Rosenbluth (std::vector <Polymer>* Polymers, std::vector <Particle>* Solvent, int x, int y, int z, bool v, bool* IMP_BOOL, double* rweight){
    ...
    switch (r) {
        ...
        case (9): 
        	if (v) {
        		printf("Performing entire polymer orientation flips. \n"); 
        	} 
        	NewPol = *Polymers; 
        	PolymerFlip (&NewPol); 
        	(*rweight) = 1;
