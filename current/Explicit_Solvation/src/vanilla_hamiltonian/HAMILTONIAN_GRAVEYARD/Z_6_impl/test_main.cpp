...
int main(int argc, char** argv) {
    ...
    Solvent_vec.reserve (1000*1000*1000); 
    for (const std::array<int,3>& loc: the_lattice ){
        Solvent_vec.push_back ( Particle (loc, 's', 0) );
    }

    // try using std::find for Solvent_c 

    std::cout << "Using std::find on Solvent_c..." << std::endl; 
    Particle test_p ( {99,98,97}, 's', 0 );
    auto start = std::chrono::high_resolution_clock::now(); 
    if ( std::find (Solvent_vec.begin(), Solvent_vec.end(), test_p ) != Solvent_vec.end() ) {
        std::cout << "Found it!" << std::endl;
    }
    else {
        std::cout << "Not present in Solvent_c." << std::endl;
    }
    
    auto stop = std::chrono::high_resolution_clock::now(); 
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds> (stop-start); 

    std::cout << "\n\nTime taken for vector find: " << duration.count() << " milliseconds.\n"; 
    std::cout << "--------------------------------------------------------------------\n\n";

    

    std::set <Particle> Solvent_set;
    for (const std::array<int,3>& loc: the_lattice ){
        Solvent_set.insert( Particle (loc, 's', 0) );
    }

    std::cout << "Using set.find on Solvent_c..." << std::endl; 
