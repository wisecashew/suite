...
bool checkConnectivity(std::vector <Polymer> PolymerVector, int x, int y, int z) {
    ...
    for (Polymer& pmer: PolymerVector){
        ...
        for (size_t i{1}; i<length; ++i){
            ...
            // std::cout << "particle index is " << i << " and coords are "; print(pmer.chain[i].coords);
            // std::cout << "particle index is " << i-1 << " and coords are "; print(pmer.chain[i-1].coords);
            // std::cout << "connection vector is "; print(connection); 
            int counter = 0; 
            for ( std::array<int,3> valid_drns: adrns){

            	if (connection == valid_drns){
                	break; 
            	}
            	counter += 1;  
...
void dumpEnergy (double sysEnergy, int step, double mm_aligned, double mm_naligned, int ms_aligned, int ms_naligned, std::string filename){
    ...
    dump_file << sysEnergy << " | " << mm_aligned + mm_naligned << " | " << mm_aligned << " | " << mm_naligned << " | " << ms_aligned + ms_naligned << " | " << ms_aligned << " | " << ms_naligned << " | " << step << "\n";
...
std::vector <Polymer> BondVibration_Rosenbluth (std::vector <Polymer>* Polymers, std::vector <Particle>* Solvent, int index, int x, int y, int z, bool* IMP_BOOL, double* rweight ){
	...
	for (Particle& p: (*Solvent)){
		if (p.coords == new_loc){
				p.coords = (*Polymers)[index].chain[m_idx].coords; 
				break;
		}
	}
...
std::vector <Polymer> ForwardReptation_Rosenbluth(std::vector <Polymer>* PolymerVector, std::vector <Particle>* SolvVector, int index, int x, int y, int z, bool* IMP_BOOL, double* rweight){
    ...
    std::array <std::array <int,3>, 26> ne_list = obtain_ne_list( NewPol[index].chain[deg_poly-1].coords, x, y, z ); 
    std::vector <std::array<int,3>> idx_v; 
...
std::vector <Polymer> BackwardReptation_Rosenbluth(std::vector <Polymer>* PolymerVector, std::vector <Particle>* SolvVector, int index, int x, int y, int z, bool* IMP_BOOL, double* rweight){
    ...
    std::array <std::array <int,3>, 26> ne_list = obtain_ne_list( NewPol[index].chain[0].coords, x, y, z ); 
    std::vector <std::array <int,3>> idx_v; 
...
void PolymerFlipLocal ( std::vector <Polymer>* Polymers, std::vector <Particle>* Solvent, int p_idx, int x, int y, int z, double* rweight, int Nsurr ){
    ...
    // number of surrounding solvent molecules Nsurr 
