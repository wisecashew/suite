...
int main(int argc, char** argv) {
    ...
    */
    ...
    auto start = std::chrono::high_resolution_clock::now(); 
    // time required using = operator 
    for (int i{0}; i<1; ++i){
        std::vector <Particle> SolventVector_ = SolventVector; 
        std::vector <Polymer>  PolymerVector_ = PolymerVector; 
    }
    auto stop = std::chrono::high_resolution_clock::now(); 
    auto duration = std::chrono::duration_cast<std::chrono::microseconds> (stop-start); 
    std::cout << "Time taken by = operator is " << duration.count() << " microseconds." << std::endl;
    
    start = std::chrono::high_resolution_clock::now(); 
    // time required using = operator 
    for (int i{0}; i<1; ++i){
        std::vector <Particle> SolventVector__; 
        std::vector <Polymer>  PolymerVector__; 
    }
    stop = std::chrono::high_resolution_clock::now(); 
    duration = std::chrono::duration_cast<std::chrono::microseconds> (stop-start); 
    std::cout << "Time taken by copy function is " << duration.count() << " microsecond." << std::endl;
