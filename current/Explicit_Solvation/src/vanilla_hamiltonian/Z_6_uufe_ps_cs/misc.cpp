...
std::array <double,11> ExtractTopologyFromFile(std::string filename){
    ...
    Emm_a ("Emm_a"), Emm_n ("Emm_n"), Ems1_a ("Ems1_a"), Ems1_n ("Ems1_n"), \
    Ems2_a ("Ems2_a"), Ems2_n ("Ems2_n"), eof ("END OF FILE"); 
        ...
        for (Particle*& p: pmer.chain){
            ...
            for ( std::array <int, 3>& loc: ne_list){
            	...
            	if ( (*LATTICE)[ lattice_index(loc, y, z) ]->ptype[0] == 'm'){
            		...
            		if ( (*LATTICE)[ lattice_index (loc, y, z) ]->ptype == "s1" ){
...
void AddSolvent_S2 (int x, int y, int z, int N, double frac, std::vector <Polymer>* Polymers, std::vector <Particle*>* LATTICE){
	...
	int nsol2 = std::floor ( (x*y*z-N)*frac );
	std::vector <int> solvent_indices; solvent_indices.reserve( x*y*z-N ); 

	// created a big vector full of indices 
	std::iota (solvent_indices.begin(), solvent_indices.end(), 0);

	// get rid of polymer indices 
	// to do that, first get monomer indices 
	std::vector <int> monomer_indices; monomer_indices.reserve(N);
	for ( Polymer& pmer: *Polymers){
		for (Particle*& p: pmer.chain){
	...
	// sort the monomer indices 
	std::sort ( monomer_indices.begin(), monomer_indices.end() ); 

	for ( std::vector<int>::reverse_iterator i = monomer_indices.rbegin(); i != monomer_indices.rend(); ++i ){
		solvent_indices.erase (solvent_indices.begin() + (*i));
	}

	// we now have all solvent_indices that are solvent 
	// shuffle them randomly, and then allocate the first nsol2 particles the {'s','2'} ptype 
