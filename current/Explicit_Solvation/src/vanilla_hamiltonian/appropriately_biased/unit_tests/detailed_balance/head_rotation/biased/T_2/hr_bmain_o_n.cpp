...
int main (int argc, char** argv) {
    ...
    // BEGIN: main loop of simulation! 
    double prob_o_to_n = 1; 
    std::vector <double> transfer_prob; 
    ...
    std::vector <std::array<int,3>> target = {{5,1,0}, {4,1,0}};
    ...
    for (int i = step_number+1; i< (step_number+max_iter+1); ++i) {
        ...
        PerturbSystem (&Polymers, &LATTICE, &E, &contacts, &attempts, &IMP_BOOL, v, &sysEnergy, &prob_o_to_n, T, &move_number, x, y, z); 
