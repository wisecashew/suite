...
void PerturbSystem (std::vector <Polymer>* Polymers, std::vector <Particle*>* LATTICE, \
	...
	bool* IMP_BOOL, bool v, double* sysEnergy, double* prob_o_to_n, double temperature, \
	...
	switch (r) {
		...
		case (2):
			...
			ChainRegrowth (Polymers, LATTICE, E, contacts, IMP_BOOL, sysEnergy, prob_o_to_n, temperature, index, x, y, z);
			*move_number    = 2; 
			(*attempts)[2] += 1;			
			break; 
...
void ChainRegrowth (std::vector <Polymer>* Polymers, std::vector <Particle*>* LATTICE, \
	std::array <double,4>* E, std::array <double,4>* contacts, bool* IMP_BOOL, \
	double* sysEnergy, double* prob_o_to_n, double temperature, int p_index, int x, int y, int z){

	// choose an index to regrow from 
	int deg_poly = (*Polymers)[p_index].deg_poly; 
	int m_index  = 5; //rng_uniform (1, deg_poly-2); 
	...
	*prob_o_to_n = 1; 
	...
	if ( growth ){
		...
		if ( rng_acc < std::exp(-1/temperature * (frontflow_energy- *sysEnergy)) * prob_n_to_o/(*prob_o_to_n) ){
			...
			// perform swaps 
			for (int i{m_index+1}; i<deg_poly; ++i){
				// std::cout << "Polymers.chain["<<i<<"]->coords = "; print((*Polymers)[p_index].chain[i-m_index-1]->coords);
				// std::cout << "new_cut[i-m_index-1] = "; print(new_cut[i-m_index-1]); 
				// std::cout << "old_cut[i-m_index-1] = "; print(old_cut[i-m_index-1]); 
