...
int main (int argc, char** argv) {
    ...
    for ( int i{0}; i<m_index; ++i ){
        old_cut.push_back ( (Polymers)[0].chain[i]->coords ); 
    }

    std::vector <std::array<int,3>> target = {{1,1,0}};
    ...
    for (int i = step_number+1; i< (step_number+max_iter+1); ++i) {
        ...
        if (IMP_BOOL){
            ...
            for (int i{0}; i<m_index; ++i){
                ...
                (LATTICE)[lattice_index (old_cut[i], y, z)]->coords = loc_m;
                (Polymers)[0].chain[i]->coords = old_cut[i];

                (LATTICE)[ lattice_index (loc_m, y, z) ] = (LATTICE)[ lattice_index (old_cut[i], y, z) ];
                (LATTICE)[ lattice_index (old_cut[i], y, z) ] = (Polymers)[0].chain[i];
            }
        }
