...
double CalculateEnergy (std::vector <Polymer>* Polymers, std::vector <Particle*>* Cosolvent, std::vector <Particle*>* LATTICE, std::array<double,8>* E, std::array<double,8>* contacts, int x, int y, int z) {
    ...
    // std::cout << "monomer energy computation took " << duration.count() << " microseconds. " << std::endl;
...
double CalculateEnergy_parallel (std::vector <Polymer>* Polymers, std::vector <Particle*>* Cosolvent, std::vector <Particle*>* LATTICE, std::array<double,8>* E, std::array<double,8>* contacts, int x, int y, int z) {
    ...
    for (dummy_idx=id; dummy_idx<NCosolvent; dummy_idx = dummy_idx+nthrds) {
        ...
        for ( std::array <int,3>& loc: ne_list ) {
    		...
    		theta_1_p = std::acos (take_dot_product ( Dir2Or[scale_arrays(invmagnitude_p, &connvec_p)] , (*Cosolvent)[dummy_idx]->orientation ) ); 
    		theta_2_p = std::acos (take_dot_product ( Dir2Or[scale_arrays(-invmagnitude_p, &connvec_p)], (*LATTICE)[lattice_index(loc, y, z)]->orientation ) ); 
...
double NeighborEnergy ( std::vector <Particle*>* LATTICE, std::array <double,8>* E, std::array <double,8>* contacts, int ss_index, int x, int y, int z){
	...
	std::array <int,3> ploc = (*LATTICE)[ss_index]->coords; 
	std::string particle_type = (*LATTICE)[ss_index]->ptype; 
	int orientation = (*LATTICE)[ss_index]->orientation; 
	...
	// DELETE THIS LATER 
	// if ( particle_type[0] != 's' ){
	//	std::cout << "BAD SOLVATION SHELL!!" << std::endl;
	//	exit (EXIT_FAILURE); 
	// }
	...
	for ( std::array <int,3>& loc: ne_list) {
		...
		if ( particle_type == "s1" ) {
			if ( (*LATTICE)[ lattice_index(loc, y, z) ]->ptype == "m1" ){
				dot_prod = take_dot_product ( orientation, (*LATTICE)[ lattice_index(loc, y, z) ]->orientation );
				if ( dot_prod > 0.54 ){
					Ei += (*E)[2];
					(*contacts)[2] += 1; 
				}
				...
				modified_direction ( &connvec, x, y, z); 
				magnitude = std::sqrt (connvec[0]*connvec[0]+connvec[1]*connvec[1]+connvec[2]*connvec[2]); 
    			theta_1 = std::acos (take_dot_product ( Dir2Or[scale_arrays(1/magnitude, &connvec)], orientation ) ); 
    			theta_2 = std::acos (take_dot_product ( Dir2Or[scale_arrays(-1/magnitude, &connvec)], (*LATTICE)[lattice_index(loc, y, z)]->orientation ) ); 
...
void SolventFlip_UNBIASED ( std::vector <Polymer>* Polymers, std::vector <Particle*>* Cosolvent, std::vector <Particle*>* LATTICE,  \
	...
	if (*IMP_BOOL){
		...
		double energy = CalculateEnergy (Polymers, Cosolvent, LATTICE, E, &c_contacts, x, y, z); 
...
void SolvationShellFlip_BIASED (std::vector <Polymer>* Polymers, std::vector <Particle*>* Cosolvent, std::vector <Particle*>* LATTICE,  \
    ...
    for ( int i{0}; i < nflip; ++i ) {
    	...
    	for ( int j{0}; j < ntest; ++j ){
    		...
    		energies [j]        = CalculateEnergy (Polymers, Cosolvent, LATTICE, E, &c_contacts1, x, y, z); 
    ...
    for ( int i{0}; i < nflip; ++i ){
    	...
    	energies[0] = CalculateEnergy (Polymers, Cosolvent, LATTICE, E, &c_contacts2, x, y, z);
    	contacts_store[0] = c_contacts2; 

    	for ( int j{1}; j < ntest; ++j ){
    		(*LATTICE)[ solvation_shell_indices[i] ]->orientation = rng_uniform(0, 25); 
    		energies[j] = CalculateEnergy (Polymers, Cosolvent, LATTICE, E, &c_contacts2, x, y, z);
...
void PolymerFlip_BIASED (std::vector <Polymer>* Polymers, std::vector <Particle*>* LATTICE,  \
	...
	// DELETE LATER
	backflow_energy = energies[0]; 
	contacts_test     = contacts_store[0]; 

	if ( backflow_energy != *sysEnergy || contacts_test != *contacts ){
    	std::cout << "Something is fucked. Energies do not match." << std::endl;
    	std::cout << "backflow_energy = " << backflow_energy << ", sysEnergy = " << *sysEnergy << std::endl;
    	std::cout << "contacts_test = "; print (contacts_test, ", "); std::cout << "contacts = "; print (*contacts); 
    	exit(EXIT_FAILURE); 
    }
    // DELETE ABOVE LATER
...
void HeadRegrowth_BIASED (std::vector <Polymer>* Polymers, std::vector <Particle*>* Cosolvent, std::vector <Particle*>* LATTICE,  \
	...
	std::array <std::array<int,3>, 26> ne_list = obtain_ne_list ((*Polymers)[p_index].chain[m_index]->coords, x, y, z); 
        ...
        double Esys = *sysEnergy;
        double Em = 0, Es = 0, Es_n = 0, Em_n = 0, energy_n = 0; 
		...
		if ( ne_list[idx_counter] == loc_m ){ 
			...
			if ( self_swap_idx < m_index ){
				...
				block_counter += 1; 
                                ...
                                // initial neighoring interactions
                                Em = NeighborEnergy ( LATTICE, E, &cm, lattice_index((*Polymers)[p_index].chain[m_index]->coords, x, y, z), x, y, z);
                                Es = NeighborEnergy ( LATTICE, E, &cs, lattice_index((*Polymers)[p_index].chain[self_swap_idx]->coords, x, y, z), x, y, z);
				// prep the swap 
				...
				// get the energy
                                Em_n = NeighborEnergy (LATTICE, E, &cm_n, lattice_index((*Polymers)[p_index].chain[m_index+1]->coords, x, y, z), x, y, z); 
                                Es_n = NeighborEnergy (LATTICE, E, &cs_n, lattice_index((*Polymers)[p_index].chain[self_swap_idx]->coords, x, y, z), x, y, z); 
                                energies [idx_counter] = Esys - (Es+Em) + (Es_n+Em_n); 
                                contacts_store [idx_counter] = add_arrays ( subtract_arrays (*contacts, add_arrays (cs, cm)), add_arrays (cm_n, cs_n) );
                                
				energy_n = CalculateEnergy (Polymers, Cosolvent, LATTICE, E, &current_contacts, x, y, z);
				if (energies[idx_counter] != energy_n || contacts_store[idx_counter] != current_contacts){
                                    
				    std::cout << "Either energy or contacts is messed up in id swap in frontflow... " << std::endl;
				    std::cout << "energies["<<idx_counter<<"] = " << energies[idx_counter] << ", energy_n = " << energy_n << ". " << std::endl; 
				    std::cout << "contacts_store[" <<idx_counter<< "] = "; print (contacts_store[idx_counter], ", "); std::cout << "c_contacts = "; print(current_contacts);
				    exit(EXIT_FAILURE);
                                }
				...
				(*LATTICE)[lattice_index(loc_m, y, z)]         = (*Polymers)[p_index].chain[m_index+1];
                        ...
                        // initial neighboring interactions 
                        Em = NeighborEnergy ( LATTICE, E, &cm, lattice_index((*Polymers)[p_index].chain[m_index+1]->coords, x, y, z), x, y, z);
                        Es = NeighborEnergy ( LATTICE, E, &cs, lattice_index((*Polymers)[p_index].chain[self_swap_idx]->coords, x, y, z), x, y, z);
...
void BackFlowFromHeadRegrowth_BIASED (std::vector <Polymer>* Polymers, std::vector <Particle*>* Cosolvent, std::vector <Particle*>* LATTICE,  \
	...
	std::array <std::array<double,8>,5> contacts_store; 
        ...
        std::array <double,8> cs1, cs2, cs1_n, cs2_n; 
        double Esys = *sysEnergy; 
        double Es = 0, Em = 0, Es_n = 0, Em_n = 0;
		...
		if ( ne_list[idx_counter] == loc_m ){
			...
			if ( self_swap_idx < m_index ){
				...
				// get the energy
                                ...
                                energies [idx_counter] = Esys - (Es+Em) + (Es_n+Em_n); 
                                contacts_store [idx_counter] = add_arrays ( subtract_arrays (*contacts, add_arrays (cs, cm)), add_arrays (cm_n, cs_n) );
			        energy_n = CalculateEnergy (Polymers, Cosolvent, LATTICE, E, contacts, x, y, z); 
			        if (energies[idx_counter] != energy_n || contacts_store[idx_counter] != current_contacts){
			            std::cout << "Either energy or contacts is messed up in id swap in frontflow... " << std::endl;
			            std::cout << "energies["<<idx_counter<<"] = " << energies[idx_counter] << ", energy_n = " << energy_n << ". " << std::endl; 
			            std::cout << "contacts_store[" <<idx_counter<< "] = "; print (contacts_store[idx_counter], ", "); std::cout << "c_contacts = "; print(current_contacts);
	...
	while ( idx_counter < 5 ){
		...
		if ( ne_list[idx_counter] == loc_m ){
			...
			if ( self_swap_idx > m_index ){
				...
				// get the energy
				energies [idx_counter] = CalculateEnergy (Polymers, Cosolvent, LATTICE, E, contacts, x, y, z); 
	...
	while ( idx_counter < 5 ){
		...
		if ( ne_list[idx_counter] == loc_m ){
			...
			for ( int i{0}; i<5; ++i){
				...
				energies    [5*idx_counter+i] = CalculateEnergy (Polymers, Cosolvent, LATTICE, E, contacts, x, y, z); 
...
void SolventExchange_BIASED (std::vector <Polymer>* Polymers, std::vector <Particle*>* LATTICE, std::array <double,8>* E, \
    ...
    std::array <double,8> cs1, cs2, cs1_n, cs2_n; 
    double Esys = *sysEnergy; 
    double Es1 = 0, Es1_n = 0, Es2 = 0, Es2_n = 0;
    // double Ef  = 0; 
    // double energy_n = 0; 
	...
	*/ 
		...
		for (int j{0}; j < ntest; ++j) { 
			...
			energies       [ j ] = Esys - (Es1 + Es2) + (Es1_n + Es2_n);  
	...
	energies [ 0 ]       = frontflow_energy - (Es1 + Es2) + (Es1_n + Es2_n); // CalculateEnergy ( Polymers, Cosolvent, LATTICE, E, &c_contacts2, x, y, z ); 
	...
	for (int j{1}; j < ntest; ++j){
		...
		Es2_n                = NeighborEnergy  (LATTICE, E, &cs2_n, solvation_shell_indices[my_idx], x, y, z);
		Es1_n                = NeighborEnergy  (LATTICE, E, &cs1_n, exc_idx, x, y, z);
		energies  [ j ]      = frontflow_energy - (Es1+Es2) + (Es1_n+Es2_n); 
