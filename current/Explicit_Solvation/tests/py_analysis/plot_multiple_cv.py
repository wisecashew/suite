...
for U in U_list:
    ...
    if U == "Uexcl":
        plt.plot (temperatures[1:], np.asarray(cv_list), linestyle='-.', marker='^', linewidth=3) 
    else:
        plt.plot (temperatures, np.asarray(cv_list), linestyle='-.', marker='^', linewidth=1, color=cm.copper(i/9) ) 
    i+=1

plt.legend(U_list, fontsize=12)
