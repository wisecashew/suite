...
if __name__ == "__main__":
    ...
    for U in U_list:
        ...
        if U=="Uexcl":
            ax.errorbar   ( temperatures, rg_mean/(args.dop**2/6), yerr=rg_std/(args.dop**2/6), linewidth=3 )
        else:
            ax.errorbar   ( temperatures, rg_mean/(args.dop**2/6), yerr=rg_std/(args.dop**2/6), linewidth=1, color=cm.copper(i/9)) 
        i+=1 
    
    ax.legend     ( U_list, loc='best', fontsize=12)
    ax.set_xlabel ( "Temperature (reduced)", fontsize=18) 
    ax.set_ylabel ( "Scaled $R_g ^2$", fontsize=18) 
    ax.set_xticks ( np.arange(0, np.max(Tmax)+2,1) ) 
    plt.savefig   ( "DOP_"+str(args.dop)+"_scaledrg.png", dpi=1200)
    plt.show() 
