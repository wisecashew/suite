...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        chi_list.append ( aux.get_chi_fh(str(U)+"/DOP_"+str(args.dop)+"/"+str(temp)+"/geom_and_esurf.txt") )
        chi = np.ones (len(temperatures))*aux.get_chi_fh(str(U)+"/DOP_"+str(args.dop)+"/"+str(temp)+"/geom_and_esurf.txt")
        ax.scatter(chi, temperatures, marker='o', edgecolors='k', c=mm_mean, cmap='coolwarm_r', label='_nolegend_', norm=divnorm)   
        print (mm_mean)
        i += 1
    ...
    plt.ylabel("Temperature (reduced)", fontsize=18)
    plt.xlabel("$\chi$", fontsize=18)
