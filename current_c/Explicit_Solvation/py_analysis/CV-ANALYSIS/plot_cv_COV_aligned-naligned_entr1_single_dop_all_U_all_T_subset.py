...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list: 
                ...
                f = df["mm_aligned"].values[-1500:]*Emm_a + df["ms1_aligned"].values[-1500:]*Ems1_a
                g = df["mm_naligned"].values[-1500:]*Emm_n + df["ms1_naligned"].values[-1500:]*Ems1_n
    ...
    print ("ms_max = ",ms_max)
    f = open ("cov-ali-nali-subset.mc", 'w')
    i=0
    ymin = 1
    chi_list = [-0.01, -0.1, -0.2]
    for U in U_list:
        rgba_color = cm.PiYG(divnorm (chi_list[i]))
        plt.errorbar ( temperatures, PLOT_DICT[U][0]/(args.dop*np.array(temperatures)**2), yerr=PLOT_DICT[U][1]/(args.dop*np.array(temperatures)**2), linewidth=1, fmt='none', capsize=2, color='k', label="_nolabel_")
        ...
        for j in range(len(PLOT_DICT[U][0])):
            f.write ("{:>2.2e} ".format(PLOT_DICT[U][0][j]/(args.dop*temperatures[j]**2)) )
        f.write ("\n")
        i += 1
