...
if __name__=="__main__":
    ...
    temperatures = aux.dir2float ( os.listdir( "U1/DOP_"+str(args.dop) ) )
    ...
    if args.ev:
        ...
        plt.errorbar ( temperatures, contacts/contacts, yerr = np.std( df["mm_tot"].values)/(np.sqrt(40)*contacts), fmt='^', markeredgecolor='k', linestyle='-', elinewidth=1, capsize=0 ) 
    ...
    for U in U_list:
        ...
        plt.errorbar(temperatures, np.asarray(mm_mean)/contacts[0], yerr=np.asarray(mm_err)/contacts[0], fmt='o', markeredgecolor='k', linestyle='-', elinewidth=1, capsize=0, color=cm.copper(i/9), label='_nolegend_')   
        i += 1
    ...
    plt.ylabel("$\\langle M_C \\rangle/ \\langle M_C \\rangle_0$", fontsize=18)
    ...
    plt.savefig("DOP_"+str(args.dop)+"SCALED_multiple_mmcorr.png", dpi=800)
