...
def infiltrate_coords_get_rg ( U, T, num, dop, coords_files, starting_index ):
    ...
    if T == 0.03 or T == 0.05:
        starting_index = 160000000
    elif T > 2.0:
        starting_index = 110000000
...
if __name__ == "__main__":    
    ...
    for U in U_list:
        chi_1 = chi_list[i]# aux.get_chi_cosolvent ( str(U)+"/DOP_32/0.1/geom_and_esurf.txt")[args.cs]
        print(chi_1)
        rgba_color = cm.PiYG (divnorm (chi_list[i]) )
        drg = np.diff (PLOT_DICT[U][0])
        dt  = np.diff (temperatures)
        print ("rg = ",PLOT_DICT[U][0])
        print ("dt =", dt)
        print ("drg =", drg)
        tx  = (np.array(temperatures[:-1])+np.array(temperatures[1:]))/2
        print ("tx = ",tx)
        ax.errorbar ( tx, (drg/dt)/rg_max, yerr= 0, linewidth=1, capsize=2, color=rgba_color, ecolor='k', fmt='none', label='_nolegend_')
        ax.plot     ( tx, (drg/dt)/rg_max, marker='o', markeredgecolor='k', \
