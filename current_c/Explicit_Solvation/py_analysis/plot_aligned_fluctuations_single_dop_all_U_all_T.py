...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list: 
                ...
                ms_list = np.hstack ( (ms_list, np.mean(df["ms1_aligned"]**2) - (np.mean(df["ms1_aligned"]))**2 ) ) # np.hstack ( (ms_list, df["ms1_tot"].values + df["ms2_tot"].values ) )
    ...
    if args.cs == 0:
        my_cmap = cm.PiYG
        sm = plt.cm.ScalarMappable( cmap=my_cmap, norm=plt.Normalize(vmin=-1000.0, vmax=1000) )
    ...
    cbar.set_ticks ( [-1000, -750, -500, -250, 0] )
