...
dop   = 32
step  = -1
frac  = aux.get_frac ("/scratch/gpfs/satyend/MC_POLYMER/SingleChainComprehensive/runs/all_regimes/cosolvent/isotropic4_dump/U3/DOP_32/1.0/geom_and_esurf.txt")
...
f = open ("/scratch/gpfs/satyend/MC_POLYMER/SingleChainComprehensive/runs/all_regimes/cosolvent/isotropic4_dump/U3/DOP_32/1.0/lattice_dump_1.mc", 'r')
...
for key in m1_dict.keys():
	for i in range (27):
		perturbed_dlist.append (s2_dict[key] + edge*v[i])
...
start = time.time()
cum_hgram = np.cumsum(hgram)/(dop*keycount)
...
fig, ax = plt.subplots()
ax.plot(rrange[:-1], cum_hgram, marker='o', markersize=2, color='steelblue')
# ax.axhline(y=1, c='k', linestyle='-')
ax.minorticks_on()
fig.savefig("rdf_m1s2.png", dpi=800)
