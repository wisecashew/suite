	...
	for T in temperatures:
		...
		for u_idx in range (idx_range):
			if u_idx == idx_range-1:
				...
				results_s1s1 = pool.starmap (aux.extract_rdf_s1s1, zip(master_U_list[u_idx*nproc:], itertools.repeat(dop), itertools.repeat(T), master_num_list[u_idx*nproc:]) )
				print ("obtained the s1s1 rdf!", flush=True)
				print ("completed idx = {}".format(u_idx), flush=True) 
			else:
				print ("in idx = {}".format(u_idx), flush=True)
				results_s1s1 = pool.starmap (aux.extract_rdf_s1s1, zip(master_U_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(dop), itertools.repeat(T), master_num_list[u_idx*nproc:(u_idx+1)*nproc]))
				print ("obtained the s1s1 rdf!", flush=True)
				print ("completed idx = {}".format(u_idx), flush=True) 
		...
		PLOT_DICT[T] = np.asarray (count_s1s1)
