...
if __name__ == "__main__":
    ...
    for U in U_list:
        ...
        for T in temperatures:
            if U!="Uexcl":
                for idx in ["1", "2", "3", "4", "5"]:
                    filename = U +"/DOP_"+str(dop)+"/"+str(T)+"/coords"+idx+".txt"
                    master_dict = get_pdict (filename, dop+2, dop+2, dop+2) 
            
                    for key in master_dict: 
                        rg_list = np.hstack ( ( rg_list, get_Rg(master_dict[key][0], dop+2, dop+2, dop+2) ) )  
                    rg_net = np.hstack( ( rg_net, rg_list[args.s:] ) )
                rg_mean = np.hstack ( ( rg_mean, np.mean ( rg_net ) ) ) 
                rg_std  = np.hstack ( ( rg_std , np.std  ( rg_net ) / np.sqrt(5) ) )
            if U == "Uexcl": 
                if T == 0.01:
                    continue
                else:
        ...
        if U=="Uexcl":
            ax.errorbar   ( temperatures[1:], rg_mean, yerr=rg_std, linewidth=1, elinewidth=0.5, capsize=2, color='black' )
        else:
            ax.errorbar   ( temperatures, rg_mean, yerr=rg_std, elinewidth=0.5, capsize=2, linewidth=1, alpha=0.5, color=cm.copper(i/9)) 
        i += 1
