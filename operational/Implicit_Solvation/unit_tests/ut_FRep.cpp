    ...
    for (size_t i=0; i<G.PolymersInGrid.size(); i++){
        for (size_t j=0; j<G.PolymersInGrid.at(i).chain.size(); j++){
            if (!(G.PolymersInGrid.at(i).chain.at(j) == G3.PolymersInGrid.at(i).chain.at(j)) ){
                ...
                std::cout << "In the OccupancyMap lens..." << std::endl; 

                std::cout << "Position of the particle now is "; print(G3.OccupancyMap.at( G3.PolymersInGrid.at(i).chain.at(j).coords ).coords);
                std::cout << "Orientation of the particle now is " <<  G3.OccupancyMap.at( G3.PolymersInGrid.at(i).chain.at(j).coords ).orientation;

                if (G.PolymersInGrid.at(i).chain.at(j).orientation != G3.PolymersInGrid.at(i).chain.at(j).orientation){
                    std::cerr << "Problem in orientation. Exiting..." << std::endl;
                    exit(EXIT_FAILURE); 
                }
                ...
                if (G.PolymersInGrid.at(i).chain.at(j).ptype != G3.PolymersInGrid.at(i).chain.at(j).ptype){
                    std::cerr << "Problem in type. Exiting..." << std::endl;
                    exit(EXIT_FAILURE); 
                }
...
/*
    ...
    for (std::map <std::vector <int>, Particle>::iterator iter=G.OccupancyMap.begin(); iter!=G.OccupancyMap.end(); ++iter){
        ...
        if ( !(G.OccupancyMap.at(key) == G3.OccupancyMap.at(key) ) ){
            ...
            std::cout << "The particle type now is " << G3.OccupancyMap.at(key).ptype << std::endl;
