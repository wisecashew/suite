...
if __name__=="__main__":
    ...
    for T in temperatures:
        ...
        for U in U_list: 
            ...
            for num in num_list: 
                ...
                ms_list = np.hstack ( (ms_list, np.mean(df["ms1_aligned"].values[-2000:]*Ems1_a + df["ms1_naligned"].values[-2000:]*Ems1_n) ) )
                mm_list = np.hstack ( (mm_list, np.mean(df["mm_aligned"].values[-2000:]*Emm_a   + df["mm_naligned" ].values[-2000:]*Emm_n ) ) )
                
            ms_err  = np.hstack ( (ms_err,  np.std(ms_list) / np.sqrt( len (num_list) ) ) )
            ms_mean = np.hstack ( (ms_mean, np.mean(ms_list) ) )
            mm_err  = np.hstack ( (mm_err,  np.std(mm_err) / np.sqrt(len (num_list) ) ) ) 
            mm_mean = np.hstack ( (mm_mean, np.mean (mm_list) ) )

        MM_DICT [T]      = mm_mean
        MM_ERROR_DICT[T] = mm_err
        MS_DICT [T]      = ms_mean 
        MS_ERROR_DICT[T] = ms_err
    ...
    for T in temperatures:
        plt.errorbar ( frac_list, MM_DICT[T], yerr=MM_ERROR_DICT[T], linewidth=1, fmt='none', ecolor=colors[i], capsize=2, color='k', label="_nolabel_")
        plt.plot     ( frac_list, MM_DICT[T], linestyle='-',  markeredgecolor='k', linewidth=3, color=colors[i], label="_nolabel_", markersize=10, marker='o')
        plt.errorbar ( frac_list, MM_DICT[T], yerr=MS_ERROR_DICT[T], linewidth=1, fmt='none', ecolor=colors[i], capsize=2, color='k', label="_nolabel_")
        plt.plot     ( frac_list, MS_DICT[T], linestyle='-',  markeredgecolor='k', linewidth=3, color=colors[i], label="_nolabel_", markersize=10, marker='o')
        i += 1
