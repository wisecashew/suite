...
parser.add_argument('--dump-file', dest='e', metavar='RH_DATA_X', help="Provide the name of the dump file.")
...
if __name__=="__main__":
    ...
    for line in f:
        ...
        if re.match (U_str, line):
            match_flag=True 
            continue 
        elif re.match ( rh_str, line ) and match_flag:
    ...
    print (T_list)
    plt.plot    ( T_list, rh_list, marker='o', markeredgecolor='k', markerfacecolor='green', linestyle='-', linewidth=3 ) 
    plt.ylabel  ( "$\\langle R_h \\rangle$" ) 
    plt.xlabel  ( "Temperature (reduced)" ) 
    plt.xticks  ( np.arange(0,11,1) ) 
    plt.legend  ( ["U = " + str(U)] ) 
    plt.title   ("N = " + str(args.dop) )
    plt.savefig ("rh_plot_dop"+str(dop)+"_"+str(U)+".png", dpi=800) 
    if args.sp:
        plt.show()
