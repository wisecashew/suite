...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list:
                df = pd.read_csv(str(U)+"/DOP_"+str(args.dop)+"/"+str(temp)+"/"+args.e+"_"+str(num), sep=' \| ', names=["energy", "mm_tot", "mm_aligned", "mm_naligned", "ms_tot", "ms_aligned", "ms_naligned", "time_step"], engine='python')
        ...
        plt.errorbar(temperatures, np.asarray(mm_mean)/(scaling), yerr=np.asarray(mm_err)/(args.dop**2), fmt='o', markeredgecolor='k', linestyle='-', elinewidth=1, capsize=0, color=cm.copper(i/9), label='_nolegend_')   
        i += 1
    ...
    if args.ev:
        ...
        plt.errorbar ( temperatures, contacts/(scaling), yerr=np.std(df["mm_tot"].values)/(scaling * np.sqrt(40)), fmt='^', markeredgecolor='k', linestyle='-', elinewidth=1, capsize=0 )
    ...
    plt.ylabel("$\\frac{ \\langle M_C \\rangle} { \\left(  \\frac{N \\cdot (N-1)}{2}  \\right) }$", fontsize=18)
    ...
    plt.savefig("DOP_"+str(args.dop)+"_SCALED_mmcorr.png", dpi=800)
