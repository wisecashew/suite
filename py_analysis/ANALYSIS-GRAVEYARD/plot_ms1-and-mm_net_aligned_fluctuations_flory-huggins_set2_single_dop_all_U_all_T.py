...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list: 
                ...
                f = df["ms1_aligned"].values[-2000:]+ 2*df["mm_aligned"].values[-2000:] # +2*df["mm_aligned"][-1500:] # / (df["ms1_tot"].values[-1500:]+2*df["mm_tot"][-1500:]) )
    ...
    for U in U_list:
        ...
        if args.cs == 0: 
            ...
            rgba_color = cm.PiYG(divnorm (chi_1))
        plt.errorbar ( temperatures, PLOT_DICT[U][0] / (np.array(temperatures)), yerr=PLOT_DICT[U][1]/ms_max, linewidth=1, fmt='none', capsize=2, color='k', label="_nolabel_")
        plt.plot     ( temperatures, PLOT_DICT[U][0] / (np.array(temperatures)), linestyle='-', marker='o',  markeredgecolor='k', linewidth=3, color=rgba_color, label="_nolabel_", markersize=10)
