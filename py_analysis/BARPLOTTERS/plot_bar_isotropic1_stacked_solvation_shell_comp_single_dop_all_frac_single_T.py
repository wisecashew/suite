...
if __name__=="__main__":
    ...
    for T in temperatures:
        ...
        for U in U_list: 
            ...
            for num in num_list: 
                ...
                ms1_list = np.hstack ( (ms1_list, df["ms1_tot"].values[-2000:] /(df["ms1_tot"].values[-2000:]+df["ms2_tot"].values[-2000:]) ) )
                ms2_list = np.hstack ( (ms2_list, df["ms2_tot"].values[-2000:] /(df["ms1_tot"].values[-2000:]+df["ms2_tot"].values[-2000:]) ) )
                
                
            ms1_err  = np.hstack ( (ms1_err , np.std (ms1_list)/np.sqrt( len ( num_list) ) ) )
            ms1_mean = np.hstack ( (ms1_mean, np.mean(ms1_list) ) )
            ms2_err  = np.hstack ( (ms2_err , np.std (ms2_list)/np.sqrt( len (num_list) ) ) )
            ms2_mean = np.hstack ( (ms2_mean, np.mean(ms2_list) ) )

        PLOT_DICT [T] = (ms1_mean, ms2_mean)
        ERROR_DICT[T] = (ms1_err , ms2_err )
    ...
    for T in temperatures:
        ...
        fig, ax = plt.subplots(num=i+1, figsize=(8,6))
        ...
        ax.legend(["$s_1$ fraction" , "$s_2$ fraction"], ncol=2)
