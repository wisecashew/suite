...
if __name__=="__main__":
	...
	energy_upd[0] = energy_upd[0] - chi_aligned  * beta * ( np.mean(df_target["mm_aligned" ].values[-s:]) - np.mean(df_model["mm_aligned"].values[-s:])   )
	energy_upd[1] = energy_upd[1] - chi_naligned * beta * ( np.mean(df_target["mm_naligned"].values[-s:]) - np.mean(df_model["mm_naligned"].values[-s:]) )

	delta_file.write ("aligned chi    = {}\n".format (chi_aligned))
	delta_file.write ("aligned update = {}\n".format (chi_aligned * beta * ( np.mean(df_target["mm_aligned"].values[-s:]) - np.mean(df_model["mm_aligned"].values[-s:]))))

	delta_file.write ("naligned chi    = {}\n".format (chi_naligned) )
	delta_file.write ("naligned update = {}\n".format (chi_naligned * beta * ( np.mean( df_target["mm_naligned"].values[-s:] ) - np.mean( df_model["mm_naligned"].values[-s:] ) ) ) )
	delta_file.close ()
	...
	file_new.write ("Emm_a = {:.10f}\n".format())
	file_new.write ("Emm_n = {:.10f}\n".format())
