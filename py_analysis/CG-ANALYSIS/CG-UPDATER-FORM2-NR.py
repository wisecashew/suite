...
if __name__=="__main__":
	...
	delta_file = open (str(T)+"/FORM2/MODEL"+str(args.m)+"/delta_new.mc", 'w')
	delta_file.write (f"<N_mm>_model - <N_mm>_target = {diff_mm}")
	delta_file.close()

	# now perform the update
	energy_upd[0] = energy_upd[0] - chi * ( np.mean ( df_target["mm_aligned"].values[-s:] )  - np.mean ( df_model["mm_aligned"].values[-s:] ) ) / ( beta * np.mean (df_model["mm_aligned"].values[-s:]**2) - beta * np.mean(df_model["mm_aligned"].values[-s:])**2  )

	energy_upd[1] = energy_upd[1] - chi * ( np.mean ( df_target["mm_naligned"].values[-s:] ) - np.mean ( df_model["mm_naligned"].values[-s:] ) ) / ( beta * np.mean (df_model["mm_naligned"].values[-s:]**2) - beta * np.mean (df_model["mm_naligned"].values[-s:])**2 )

	# print ("nalign num   =", ( np.mean ( df_target["mm_naligned"].values[-s:] ) - np.mean ( df_model["mm_naligned"].values[-s:] ) ) )
	# print ("nalign denom =", ( beta * np.mean (df_model["mm_naligned"].values[-s:]**2) - beta * np.mean (df_model["mm_naligned"].values[-s:])**2  ))
