...
parser.add_argument('--Hmix', dest='Hmix', action='store', nargs='+', type=str, help='Enter enthalpies you want plotted.')
...
def get_contacts (U, dop, H, num, s):

	
	df = pd.read_csv(str(U)+"/DOP_"+str(dop)+"/"+H+"/energydump_"+str(num)+".mc", sep=' \| ', names=["energy", "mm_tot", "mm_aligned", "mm_naligned", "ms1_tot", "ms1_aligned", "ms1_naligned", "ms2_tot", "ms2_aligned", "ms2_naligned", "ms1s2_tot",  "ms1s2_aligned", "ms1s2_naligned", "time_step"], engine='python', skiprows=0) 
...
if __name__=="__main__":
	...
	for H in enthalpies:
		...
		for U in U_list:
			...
			num_list = list (np.unique (aux.dir2nsim (os.listdir (U + "/DOP_" + str(dop) + "/" + H ) ) ) )
		...
		for u_idx in range(idx_range):
			if u_idx == idx_range-1:
				results = pool.starmap ( get_contacts, zip( master_U_list[u_idx*nproc:], itertools.repeat(dop), itertools.repeat(H), master_num_list[u_idx*nproc:], itertools.repeat(s) ) )
			else:
				results = pool.starmap ( get_contacts, zip( master_U_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(dop), itertools.repeat(H), master_num_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(s) ) )
