...
def get_solvation_shell (U, dop, f, num, starting_index):
	...
	for U in U_list:
		...
		for u_idx in range(idx_range):
			if u_idx == idx_range-1:
				solv    = pool.starmap ( get_solvation_shell, zip( itertools.repeat(U), itertools.repeat(dop), master_frac_list[u_idx*nproc:], master_num_list[u_idx*nproc:], itertools.repeat(s) ) )
			else:
				solv = pool.starmap ( get_solvation_shell, zip( itertools.repeat(U), itertools.repeat(dop), master_frac_list[u_idx*nproc:(u_idx+1)*nproc], master_num_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(s) ) )
			...
			for k in range( len( master_frac_list[u_idx*nproc:(u_idx+1)*nproc] ) ):
				contacts_dict[master_frac_list[u_idx*nproc+k]].append (solv[k])

		CONTACTS_DICT["U"].extend([U]*len(frac_list))
		CONTACTS_DICT["x"].extend(frac_list)
		for f in frac_list:
			CONTACTS_DICT["SOLVATION"].append(np.mean(contacts_dict[f]))
