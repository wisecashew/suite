...
if __name__=="__main__":
	...
	enthalpies = args.H # np.unique(df_real["H"])
	print (df_id)
	...
	for k in range( 4 ): # len(keys)):
		for i in range(len(enthalpies)):
			...
			ax[0][0].plot (df_id["x"].values, p_diff, marker='o', linewidth=3/2, markersize=8/2, color=color_finder(enthalpies[i]), markeredgecolor='k')
			# ax[0][0].plot (df_id["x"].values, p_diff, marker='o', linewidth=3/2, markersize=8/2, color=cm.greys(divnorm(enthalpies[i])), markeredgecolor='k')
