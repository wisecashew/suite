...
if __name__=="__main__":
    ...
    for U in U_list:
        ...
        for temp in temperatures: 
            ...
            for num in num_list: 
                ...
                f = (df["mm_aligned" ].values[-1000:]*Emm_a  + df["mm_naligned" ].values[-1000: ]*Emm_n)
                # g = df["ms1_aligned"].values[-1000:]*Ems1_a + df["ms1_naligned"].values[-1000: ]*Ems1_n
                # f= f/np.mean(f); g = g/np.mean(g);
                # ms_list = np.hstack ( ( ms_list, np.mean ( f*g )/np.m -1 ) )

                ms_list = np.hstack ( ( ms_list, (np.mean ( f*f ) - np.mean(f)*np.mean(f))/temp**2 ) )
    ...
    ymin = 1
    ms_max = 1
    chi_list = [0.1, 0, -0.2]# [0.1, 0.05, 0.01, 0.001, 0, -0.001, -0.01, -0.1, -0.2]
    for U in U_list:
        ...
        plt.plot     ( temperatures, PLOT_DICT[U][0] , linestyle='-', marker='o',  markeredgecolor='k', linewidth=2, color=rgba_color, label="_nolabel_", markersize=6)
    ...
    ax.set_xscale('log')
    ax.set_xticks ([0.01, 0.1, 1.0, 10, 100.0])
