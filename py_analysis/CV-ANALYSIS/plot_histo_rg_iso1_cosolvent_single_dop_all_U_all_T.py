...
if __name__ == "__main__":    
    ...
    for T in temperatures:
        ...
        for u_idx in range(idx_range):
            if u_idx == idx_range-1:
                results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_rg, zip( master_U_list[u_idx*nproc:] , itertools.repeat(T), master_num_list[u_idx*nproc:], itertools.repeat(dop), itertools.repeat(coords_files), master_index_list[u_idx*nproc:] ) ) # itertools.repeat(starting_index) ) )
            else:
                results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_rg, zip( master_U_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(T), master_num_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(dop), itertools.repeat(coords_files), master_index_list[u_idx*nproc:(u_idx+1)*nproc] ) )# itertools.repeat(starting_index) ) )
            ...
            for k in range( len( master_U_list[u_idx*nproc:(u_idx+1)*nproc] ) ):
                ...
                rg_dict[ master_U_list [u_idx*nproc + k] ].extend( results[k] )
    ...
    for U in U_list:
        ...
        ax.hist ( rg_dict[U], range=(1, 6), bins=np.arange(1,6,0.2), density=True, color='steelblue')
        ax.set_ylim (0, 4.5)
        plt.savefig ( args.pn+"_"+str(frac_list[i])+".png", bbox_inches='tight', dpi=1200 )
        i += 1
