...
parser = argparse.ArgumentParser(description="Read a trajectory file and obtain the H-bonds histogram.")
...
if __name__=="__main__":
	...
	for idx in range(0, 40):
		hbonds_over_T[idx] = [] # list()

	DF_list = [] # list()
	print(f"Going to compile \"hbonds.n.dump\".", flush=True)
	for idx in range(0, 40):
		df = pd.read_csv("TOT_HBONDS/hbonds."+str(idx)+".dump", sep='\s+', names=["timestep", "tot", "w_Op", "w_Np", "w_Ow"], engine="python", skiprows=2001)
		DF_list.append(df)
		...
		for line in output_file:
			if re.match(r"^[0-9]+", line):
				...
				if np.sum(timesteps==ts) == 1:
					hbonds_over_T[Tidx].extend(DF_list[replica_id]["tot"].values[timesteps==ts].tolist())
		output_file.close()
	...
	for Tidx in range(0, 40):
		...
		ax  = plt.axes()
		print(f"number of samples = {len(hbonds_over_T[Tidx])}.")
		bin_centers, mean_hist, error_hist = chunked_histogram(hbonds_over_T[Tidx], num_chunks, 30, hrange)
		mean_hbonds, err_hbonds = block_averaging(hbonds_over_T[Tidx], num_chunks)
		avg_dumpfile.write(f"{T_range[Tidx]:.2f} {mean_hbonds:.2f} {err_hbonds:.2f}\n")
		ax.errorbar(bin_centers, mean_hist, yerr=error_hist, marker='o', markeredgecolor='k', c="lavender")
		ax.set_title(f"T = {T_range[Tidx]:.2f} K")
		ax.set_xlabel("H-Bonds (count)")
		...
		fig.savefig("TOT_IMAGES/HBONDS/"+args.img+f"n-{num_chunks}_{T_range[Tidx]:.2f}.png", dpi=1200, bbox_inches="tight")
		plt.close(fig)
	avg_dumpfile.close()
