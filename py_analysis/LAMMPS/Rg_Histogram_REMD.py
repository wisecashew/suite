...
if __name__=="__main__":
	...
	DF_list = [] # list()
	print(f"Going to compile \"rg.n.dump\".", flush=True)
	for idx in range(0, 40):
		df = pd.read_csv("TOT_RG_FILES/rg."+str(idx)+".dump", sep='\s+', names=["timestep", "rg", "rgx", "rgy", "rgz"], engine="python", skiprows=2000)
		DF_list.append(df)
	...
	for Tidx in range(0, 40):
		if Tidx < 39:
			...
			print(f"{T_range[Tidx]:.2f}." , end='', flush=True)
		output_file = open("TOT_OUTPUT_FILES/output.dump", 'r')
	...
	for Tidx in range(0, 40):
		...
		bin_centers, mean_hist, error_hist = chunked_histogram(RG_over_T[Tidx], num_chunks, 50, hrange)
		mean_rg, err_rg = block_averaging(RG_over_T[Tidx], num_chunks)
		avg_dumpfile.write(f"{T_range[Tidx]:.2f} {mean_rg:.2f} {err_rg:.2f}\n")
		ax.errorbar(bin_centers, mean_hist, yerr=error_hist, marker='o', markeredgecolor='k', c="steelblue")
		ax.axvline(x=12.5, ls="--", c="darkred")
		ax.set_title(f"T = {T_range[Tidx]:.2f} K")
		ax.set_xlabel("Rg (angstrom)")
		...
		fig.savefig("TOT_IMAGES/RG/"+args.img+f"n-{num_chunks}_{T_range[Tidx]:.2f}.png", dpi=1200, bbox_inches="tight")
		plt.close(fig)
	avg_dumpfile.close()
