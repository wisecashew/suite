...
if __name__=="__main__":
	...
	for idx in range(0, 40):
		contacts_over_T[idx]    = dict()
		contacts_over_T[idx]["Nmm" ] = []
		contacts_over_T[idx]["Nms" ] = []
		contacts_over_T[idx]["Nmsa"] = []
		contacts_over_T[idx]["Ns"  ] = []

	DF_list = []
	for idx in range(0, 40):
		df = pd.read_csv("TOT_LATT/contacts."+str(idx)+".dump", sep='\s+', names=["timestep", "Nmm", "Nms", "Nmsa", "Ns"], engine="python", skiprows=200)
		DF_list.append(df)
		...
		for line in output_file:
			if re.match(r"^[0-9]+", line):
				...
				if np.sum(timesteps==ts) == 1:
					contacts_over_T[Tidx]["Nmm" ].extend(DF_list[replica_id]["Nmm" ].values[timesteps==ts].tolist())
					contacts_over_T[Tidx]["Nms" ].extend(DF_list[replica_id]["Nms" ].values[timesteps==ts].tolist())
					contacts_over_T[Tidx]["Nmsa"].extend(DF_list[replica_id]["Nmsa"].values[timesteps==ts].tolist())
					contacts_over_T[Tidx]["Ns"  ].extend(DF_list[replica_id]["Ns"  ].values[timesteps==ts].tolist())
		output_file.close()
	...
	for Tidx in range(0, 40):
		...
		ax  = plt.axes()
		avg_dumpfile.write(f"{T_range[Tidx]:.2f} ")
		for idx,key in enumerate(contacts_over_T[Tidx]):
			bin_centers, mean_hist, error_hist = chunked_histogram(contacts_over_T[Tidx][key], num_chunks, 50, hrange)
			ax.errorbar(bin_centers, mean_hist, yerr=error_hist, marker='o', markeredgecolor='k', c=cols[idx], label=key)
			mean_, err_ = block_averaging(contacts_over_T[Tidx][key], num_chunks)
			avg_dumpfile.write(f"{mean_:.2f} {err_:.2f} ")
		avg_dumpfile.write("\n")
		...
		fig.savefig("TOT_IMAGES/CONTACTS/"+args.img+f"_n{num_chunks}_{T_range[Tidx]:.2f}.png", dpi=1200, bbox_inches="tight")
		plt.close(fig)
	avg_dumpfile.close()
