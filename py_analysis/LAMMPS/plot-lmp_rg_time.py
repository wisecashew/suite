...
if __name__=="__main__":
	...
	df = pd.read_csv(args.df, engine="python", names=["time", "rg", "rgx", "rgy", "rgz"], sep='\s+', skiprows=2)
	fig, ax = plt.figure(num=0, figsize=(3,3)), plt.axes()
	
	ax.plot(df["time"].values, df["rg"].values, c="steelblue", marker='o', ls='--', lw=1, markersize=4, mec='k')
	ax.grid()
