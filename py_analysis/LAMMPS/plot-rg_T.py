...
if __name__=="__main__":
	...
	df = pd.read_csv(args.df, engine="python", names=["T", "rg", "error"], sep='\s+')
	print(df)
	fig, ax = plt.figure(num=0, figsize=(3,3)), plt.axes()
	
	ax.errorbar(df['T'].values, df["rg"].values, yerr=df["error"].values, c="steelblue", marker='o', ls='--', lw=1, markersize=4, mec='k')
