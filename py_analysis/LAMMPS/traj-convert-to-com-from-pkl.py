	...
	for idx,ts in enumerate(coarse_traj_info):
		...
		for molid in coarse_traj_info[ts]["molid"]:
			for sr_atom, atom in enumerate(coarse_traj_info[ts]["molid"][molid]["coords"]):
				srno += 1
				if molid == 1:
					f.write(f'{srno} {1} {atom[0]} {atom[1]} {atom[2]}\n')
				else:
					f.write(f'{srno} {2} {atom[0]} {atom[1]} {atom[2]}\n')
	f.close()
...
if __name__=="__main__":
	...
	start = time.time()
	print("Creating the topology objects...")

	with open('sim_info.pkl', 'rb') as handle:
		sim_info  = pickle.load(handle)

	print(f"type(sim_info) = {type(sim_info)}")

	with open('traj_info.pkl', 'rb') as handle:
		traj_info = pickle.load(handle)

	print(f"type(traj_info) = {type(traj_info)}")
