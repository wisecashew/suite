...
parser.add_argument('--e', dest='e', metavar='energydump', action='store', type=str, help='Name of energy dump file to parse information.', default='energydump') 
parser.add_argument('--o', dest='o', metavar='orientation', action='store', type=str, help='Name of shell dump file.', default='orientation') 
...
parser.add_argument('--key',   dest='key', metavar='key',   action='store', type=str, help='Name of key.', default=None) 
...
if __name__=="__main__":
	...
	for U in U_list:
		...
		fracs   = aux.dir2float(os.listdir(str(U)+"/DOP_"+str(args.dop)))
		for frac in fracs:
			skip = 0
			prop_list = np.asarray ([])
			...
			for num in num_list:
				df_sc = pd.read_csv(str(U)+"/DOP_"+str(args.dop)+"/"+str(frac)+"/"+args.o+"_"+str(num)+".mc", sep=' \| ', names=["total", "total_s", "total_c", "aligned_sc", "misaligned_sc", "timestep"], engine='python', skiprows=skip)
				df_ms = pd.read_csv(str(U)+"/DOP_"+str(args.dop)+"/"+str(frac)+"/"+args.e+"_"+str(num)+".mc", sep=' \| ', \
				names=["energy", "mm_tot", "mm_aligned", "mm_naligned", "ms1_tot", "ms1_aligned", "ms1_naligned", \
				"ms2_tot", "ms2_aligned", "ms2_naligned", "ms1s2_tot",  "ms1s2_aligned", "ms1s2_naligned", "time_step"], engine='python', skiprows=skip)
				prop_list = np.hstack((prop_list, df_sc["aligned_sc"]/df_ms["ms1_aligned"]))
	...
	# ax.set_yticks(np.arange(0.1,0.9,0.2))
	ax.set_xticklabels([])
	# ax.set_yticklabels([])
	ax.minorticks_on()
