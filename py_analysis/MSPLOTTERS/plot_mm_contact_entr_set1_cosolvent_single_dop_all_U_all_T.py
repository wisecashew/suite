...
if __name__=="__main__":
	...
	for U in U_list:
		...
		ms_err  = np.asarray([])
		ms_mean = np.asarray([])
		temperatures = [0.01, 0.03, 0.05, 0.09, 0.1, 0.3, 0.5, 0.9, 1.0, 2.5, 5.0, 10.0, 17.5, 25.0, 50.0, 100.0] # aux.dir2float ( os.listdir( str(U) +"/DOP_"+str(args.dop) ) )
	...
	for U in U_list:
		...
		plt.plot     ( temperatures, PLOT_DICT[U][0] / ms_max, linestyle='-', marker='o',  markeredgecolor='k', linewidth=3, color=rgba_color, label="_nolabel_", markersize=10, zorder=10, clip_on=False)
		i += 1
