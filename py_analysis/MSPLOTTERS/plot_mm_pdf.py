...
parser.add_argument('--T', dest='T', action='store', nargs='+', type=str, help='Name of temperature.', default=[]) 
...
if __name__=="__main__":
	...
	alpha, bbeta, loc, scale = beta.fit(mm_list, floc=dop-1, fscale=208-31)
	print(f"beta fit: alpha = {alpha} , beta = {bbeta},   loc = {loc},   scale = {scale}",   flush=True)

	scaled_mm_list = mm_list - (dop-1)
	scaled_mm_list = scaled_mm_list/np.max(scaled_mm_list)
	salpha, sbeta, sloc, sscale = beta.fit(mm_list, floc=0, fscale=1)
