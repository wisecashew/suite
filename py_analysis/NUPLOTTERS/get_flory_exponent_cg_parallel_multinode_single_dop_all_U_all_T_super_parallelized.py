...
def get_starting_ind ( U, T, num, dop, dumpfile):
    ...
    df = pd.read_csv(filename, sep=' \| ', names=["energy", "mm_tot",  "ms1_tot", "time_step"], engine='python', skiprows=0)
    L = len(df["energy"])

    return int(df["time_step"].values[0])
...
def get_avg_flory (U, T, num, dop, coords_file, starting_index, d1, d2):
	...
	# except ValueError:
	# 	model.fit (x,y)
	# 	print (f"U = {U}, T = {T}, num = {num}, dop = {dop}, coords = {coords_file}, start = {starting_index}, d1 = {d1}, d2 = {d2}", flush=True)
	...
	plt.savefig ("RAW-FITS-"+str(dop)+".png", dpi=1200, bbox_inches="tight")
	...
	return model.coef_[0] # np.mean (nu)
...
if __name__ == "__main__":
    ...
    nproc          = args.nproc
    starting_index = 0
    delta_list = list(np.arange (args.d1, args.d2+1))
