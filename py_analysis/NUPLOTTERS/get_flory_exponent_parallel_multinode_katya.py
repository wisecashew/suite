...
def get_starting_ind ( U, frac, num, dumpfile):
    filename = U + "/frac_" + str(frac) + "/" + dumpfile + "_" + str(num) + ".mc"
...
def get_avg_amounts (U, frac, num, dop, coords_file, starting_index, d1, d2):
	...
	y = []
	starting_index = get_starting_ind (U, frac, num, "energydump")
	for delta in x:
		y.append ( aux.single_sim_flory_exp_katya (U, frac, num, dop, coords_file, starting_index, delta) )
...
if __name__ == "__main__":
    ...
    for U in U_list:
        ...
        for frac in frac_list:
            num_list = list(np.unique ( aux.dir2nsim (os.listdir (str(U) + "/frac_" + frac ) ) ) )
        ...
        for uidx in range(idx_range):
            if uidx == idx_range-1:
                results = pool_list[ 0 ] .starmap ( get_avg_flory, zip( itertools.repeat(U), master_frac_list[uidx*nproc:], master_num_list[uidx*nproc:], itertools.repeat(dop), itertools.repeat(coords_files), itertools.repeat(starting_index), itertools.repeat(args.d1), itertools.repeat(args.d2) ) )
            else:
                results = pool_list[ 0 ] .starmap ( get_avg_flory, zip( itertools.repeat(U), master_frac_list[uidx*nproc:(uidx+1)*nproc], master_num_list[uidx*nproc:(uidx+1)*nproc], itertools.repeat(dop), itertools.repeat(coords_files), itertools.repeat(starting_index), itertools.repeat(args.d1), itertools.repeat(args.d2) ) )
        ...
        for frac in np.unique (master_frac_list):
            DB_DICT["U"].append (U)
            DB_DICT["x"].append (float(frac))
            DB_DICT["nu_mean"].append (np.mean (y_dict[frac]) )
            DB_DICT["nu_err" ].append (np.std  (y_dict[frac]) / np.sqrt( len( y_dict[frac] ) ) )
