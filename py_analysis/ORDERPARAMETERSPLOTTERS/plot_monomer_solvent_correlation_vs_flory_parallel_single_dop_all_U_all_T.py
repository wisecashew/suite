...
if __name__=="__main__":
    ...
    for U in U_list: 
        ...
        print (nu)
        print (PLOT_DICT[U][0]/max_op)
        ax.errorbar ( nu["nu_mean"]/2, PLOT_DICT[U][0]/max_op, yerr=PLOT_DICT[U][1]/max_op, fmt='none', linestyle='-', elinewidth=1, capsize=2, linewidth=1, color=rgba_color, label='_nolegend_')
        ax.plot     ( nu["nu_mean"]/2, PLOT_DICT[U][0]/max_op, marker='o', markeredgecolor='k', linestyle='-', linewidth=2, c=rgba_color, label='_nolegend_', markersize=10)
        i += 1
    ...
    # ax.set_xticks (np.logspace(int(np.log10(np.min(temperatures))),int( np.log10(np.max(temperatures)) ), 5))
    # ax.set_xticklabels (ax.get_xticks() )# ["$\mathbf{10^{-2}}$", "$\mathbf{10^{-1}}$", "$\mathbf{10^0}$", "$\mathbf{10^1}$", "$\mathbf{10^2}$"])
