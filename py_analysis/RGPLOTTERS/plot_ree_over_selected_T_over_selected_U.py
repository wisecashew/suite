...
parser.add_argument('--dop-suffix',   metavar='DOP',  dest='dsuf',           type=str,       action='store', help='DOP suffix.')
...
parser.add_argument('--colors',       metavar='cols', dest='cols',           action='store', nargs='+',      type=str,   help='Enter colors.')
parser.add_argument('-nproc',         metavar='N',    type=int,              dest='nproc',   action='store', help='Request these many proccesses.')
parser.add_argument('--coords',       dest='c',       metavar='coords.txt',  action='store', type=str,       help='Name of coords dump file to parse information.', default='coords.txt')
parser.add_argument('--energy',       dest='energy',       metavar='energy.mc',  action='store', type=str,       help='Name of energy dump file to parse information.', default='coords.txt')
...
def infiltrate_coords_get_ree ( U, T, num, dop, coords_files, starting_index ):
	...
	ree = aux.get_Rg(master_dict, edge, edge, edge) 
	return ree
...
##################################
	...
	for U in U_list:
		...
		for T in temperature_list:
			...
			for num in num_list:
				master_index_list.append (get_starting_ind (U, T, num, dop, args.energy) )
		...
		for u_idx in range (idx_range):
			if u_idx == idx_range-1:
				results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_ree, zip( itertools.repeat(U), master_temperature_list[u_idx*nproc:], master_num_list [u_idx*nproc:], itertools.repeat (dop), itertools.repeat (coords_files), master_index_list[u_idx*nproc:] ) )
			else:
				results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_ree, zip( itertools.repeat(U), master_temperature_list[(u_idx)*nproc:(u_idx+1)*nproc], master_num_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(dop), itertools.repeat(coords_files), master_index_list[u_idx*nproc:(u_idx+1)*nproc] ) )
			...
			for k in range( len( master_temperature_list[u_idx*nproc:(u_idx+1)*nproc] ) ):
				ree_dict[master_temperature_list[u_idx*nproc + k]].append( results[k] )

		for temp in temperature_list:
			ree_mean.append( np.mean ( ree_dict[temp] ) )
			ree_std.append ( np.std  ( ree_dict[temp] ) / np.sqrt(ntraj_dict[temp] ) )

		PLOT_DICT [U] = ( np.asarray(ree_mean), np.asarray(ree_std) )
	...
	for U in U_list:
		print(f"Mean R_ee is: {np.mean(PLOT_DICT[U][0])}", flush=True)
		ax.errorbar ( temperature_list, PLOT_DICT[U][0], yerr= PLOT_DICT[U][1], linewidth=1, capsize=2, color='k', fmt='none', label='_nolegend_')
		ax.plot     ( temperature_list, PLOT_DICT[U][0], marker='o', markeredgecolor='k', linestyle='-', linewidth=3/2, label=f'{U}', markersize=4, c=args.cols[i] ) 
		i += 1
