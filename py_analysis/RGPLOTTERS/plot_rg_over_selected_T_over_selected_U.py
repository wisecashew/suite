...
######
	...
	rg_max = 1; # (dop ** (0.588))/2 # (1+np.sqrt(2)+np.sqrt(3))/(3*6**0.5) * (dop**0.57) 
# instantiating pool
	nproc = args.nproc
	...
	for U in U_list:
		print(f"Mean Rg is: {np.mean(PLOT_DICT[U][0])}", flush=True)
		...
		ax.plot     ( temperature_list, PLOT_DICT[U][0]/rg_max, marker='o', markeredgecolor='k', linestyle='-', linewidth=3/2, label=f'{U}', markersize=4, c=args.cols[i] ) 
		i += 1
