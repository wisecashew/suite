...
def infiltrate_coords_get_rg ( U, num, dop, coords_files, starting_index ):

    filename    = U + "/DOP_" + str(dop) + "/E_-1_-1_-7/"+ coords_files + "_" + str(num)+".mc" 
...
if __name__ == "__main__":
    ...
    for T in temperatures:
        ...
        for u_idx in range (idx_range):
            if u_idx == idx_range-1:
                results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_rg, zip( master_U_list[u_idx*nproc:], master_num_list [u_idx*nproc:], itertools.repeat (dop), itertools.repeat (coords_files), master_index_list[u_idx*nproc:] ) )
            else:
                results = pool_list[ 0 ] .starmap ( infiltrate_coords_get_rg, zip( master_U_list[(u_idx)*nproc:(u_idx+1)*nproc], master_num_list[u_idx*nproc:(u_idx+1)*nproc], itertools.repeat(dop), itertools.repeat(coords_files), master_index_list[u_idx*nproc:(u_idx+1)*nproc] ) )
    ...
    for T in temperatures:
        ...
        ax.plot     ( frac_list, PLOT_DICT[T][0]/rg_max, marker='o', markeredgecolor='k', linestyle='-', linewidth=3/2, c=cm.coolwarm(divnorm(T)), label='_nolegend_', markersize=4 ) 
        i += 1
