...
if __name__ == "__main__":    
    ...
    for U in U_list:
        ...
        for uidx in range(3):
            # X =   zip( itertools.repeat(U), mtemp_list[uidx], mnum_list[uidx], itertools.repeat(dop), itertools.repeat(coords_files), itertools.repeat(starting_index) ) 
            results = pool_list[ shitty_dict[uidx] ] .starmap ( aux.infiltrate_coords_get_rg, zip( itertools.repeat(U), mtemp_list[uidx], mnum_list[uidx], itertools.repeat(dop), itertools.repeat(coords_files), itertools.repeat(starting_index) ) )
            ...
            for T in np.unique (mtemp_list[uidx]):
                ...
                rg_std.append ( np.std  ( rg_dict[T] )/ np.sqrt( 10 ) ) 
