...
def dir2nsim (list_of_dirs, energy_file="energydump"):
	l = [] 
	for dir_name in list_of_dirs:
		r = re.findall ("^"+energy_file+"_[0-9]+.mc$", dir_name) 
		if ( r ):
			x = re.findall("\d+", r[0] ) 
			l.append ( int (x[0] ) )
	l.sort() 
...
def get_Ree (master_dict, xlen, ylen, zlen):
	...
	for key in master_dict: 
		coord_arr = unfuck_polymer(master_dict[key][0], xlen, ylen, zlen)
		ree += np.linalg.norm(coord_arr[-1]-coord_arr[0])
		count += 1
...
def plot_rg_parallelized_singletrajectory ( U, T, dop, starting_index, traj_num, coords_file ): 
	...
	fig.savefig("equi_test.png", dpi=1200, bbox_inches="tight")
