...
if __name__=="__main__":
    ...
    def make_heatmap(x, y, T, g, pv, linrange):
        ...
        for x_idx in range (linrange):
            for y_idx in range(linrange):
                chi_T = chi (x[x_idx, y_idx], y[x_idx, y_idx], E_ms_a, E_ms_n, g, pv, T_range)
                chi_min = np.min(chi_T) # * T_range[np.argmin(chi_T)]
                chi_max = np.max(chi_T) # * T_range[np.argmax(chi_T)]
                z[x_idx, y_idx] = np.sign(chi_min * chi_max) * np.max(np.abs(chi_T))
    ...
    plt.savefig (f"phases-gc-g-{g}-pv-{pv}.png", bbox_inches='tight', dpi=1200)
