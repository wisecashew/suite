...
if __name__=="__main__":
	...
	fig   = plt.figure(figsize=(args.fs[0],args.fs[1]), constrained_layout=True)
	ax    = plt.axes()
	start = time.time()
	...
	for d_ms in DELTA_ms_list:
		...
		for e_ms_n in E_ms_n_list:
			...
			ax.set_xticks([args.Emmnllim, args.Emmnulim])
			ax.set_yticks([args.Dmmllim, args.Dmmulim])
			ax.set_xticklabels ([])
			ax.set_yticklabels ([])
			# ax.minorticks_on()
	...
	if "." in args.img:
		...
		img = args.img
	fig.savefig (img, dpi=2000, bbox_inches="tight")
