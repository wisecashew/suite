...
if __name__=="__main__":
    ...
    T           = np.hstack((np.logspace (-3, np.log10(60), int(1e+7) ), np.linspace (0.05,0.15, int(1e+6)), np.linspace (0.1, 1.0, int(1e+6) ) ) )
    ...
    for i in range(len(param_list)):
        ...
        arm1     = arm1 [arm1 > 0]
        if i == 3:
            ax.plot  (arm1, T1, lw=1.5, markersize=0, c=args.colors[i], ls='--', solid_capstyle="round",label="_nolabel_")
        else:
