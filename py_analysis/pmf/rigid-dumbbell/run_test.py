...
def _pmf_PS_numba(R_vals, d,
				  theta_grid, phi_grid, sin_theta,
				  dtheta, dphi,
				  sigma_AP, eps_AP,
				  sigma_BP, eps_BP,
				  beta):
...
if __name__=="__main__":
	...
	# get the object down
	thermo = RigidDumbbell.RigidDumbbell(d=0.5, sigmas=sigmas, epsilons=epsilons, temps=temp_list)
	thermo._setup_grids()
	...
	for idx,temp in enumerate(temp_list):
		start = time.time()
		W = _pmf_PS_numba(thermo.R_vals, thermo.d,
								thermo.theta_grid, thermo.phi_grid,
								thermo.sin_theta,
								...
								1/thermo.temps[idx])
		if idx % 5 == 0:
			axs.plot(thermo.R_vals, W, marker="^", markersize=6, lw=1, ls='--', mec='k')
		stop = time.time()
	...
	fig.savefig("pmf_W_PS.png", dpi=1200)
