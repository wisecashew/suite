...
parser.add_argument('-U', dest='U', action='store', type=str, nargs='+', help='Provide potential energy surface.')
parser.add_argument('--Hmix', dest='Hmix', action='store', nargs='+', type=str, help='Provide potential energy surface.')
...
if __name__=="__main__": 
	...
	for U in U_list:
		...
		for H in Hmix:
			...
			num_list = np.unique ( aux.dir2nsim ( os.listdir ( str(U) + "/DOP_" + str(args.dop) + "/" + H ) ) )

			for num in num_list: 
				df = pd.read_csv( str(U)+"/DOP_"+str(args.dop)+"/"+H+"/energydump_"+str(num)+".mc", sep=' \| ', names = name_list, engine='python', skiprows=args.s) 
				net_energy = df["energy"].values[-1]
				if  net_energy < min_energy:
					print (num)
					min_energy   = net_energy
					energy_dump  = str(U)+"/DOP_"+str(args.dop)+"/"+H+"/energydump_"+str(num)+".mc"
					coords_dump  = str(U)+"/DOP_"+str(args.dop)+"/"+H+"/coords_"+str(num)+".mc"
					lattice_dump = str(U)+"/DOP_"+str(args.dop)+"/"+H+"/lattice_dump_"+str(num)+".mc"
			new_energy = 0
			for num in num_list:

				destination_energy_dump  = str(U)+"/DOP_"+str(args.dop)+"/"+H+"/"+args.e+"_"+str(num)+".mc"
				destination_coords_dump  = str(U)+"/DOP_"+str(args.dop)+"/"+H+"/coords_"+str(num)+".mc"
				destination_lattice_dump = str(U)+"/DOP_"+str(args.dop)+"/"+H+"/lattice_dump_"+str(num)+".mc"
