	...
	def perform_first_binodal_blast (self, phi_upper, phi_lower, center, crit_point):
		...
		for idx, pu in enumerate(phi_upper):
			...
			for jdx, pl in enumerate(phi_lower):
				root_ = root(mu_equations, [pu[1], pl[0], pl[1]], tol=1e-9, method='broyden1')
				if not root_.success: # (np.abs(np.array(mu_equations(root_)))>1e-13).any():
					continue
				else:
					p1 = np.array([pu[0], root_.x[0]])
					p2 = np.array([root_.x[1], root_.x[2]])				
					if np.linalg.norm(p1-p2) < 1e-3:
						...
						if abs(p1[0]-p2[1])>1e-3 or abs(p1[1]-p2[0])>1e-3:
							...
							# print(f"seed was phi_s = {pu[0]}, guess was phi_p = {pu[1]}, phi = {pl[0], pl[1]}", flush=True)
							# print(f"delta_mu_init = {mu_equations([pu[1], pl[0], pl[1]])}, delta_mu_fin = {mu_equations([root_.x[0], root_.x[1], root_.x[2]])}", flush=True)
						break
		...
		for idx, pu in enumerate(phi_upper):
			...
			for jdx, pl in enumerate(phi_lower):
				root_ = root(mu_equations, [pu[0], pl[0], pl[1]], tol=1e-9, method='broyden1')
				if not root_.sucess: # (np.abs(np.array(mu_equations(root_)))>1e-13).any():
					continue
				else:
					p1 = np.array([root_.x[0], pu[1]])
					p2 = np.array([root_.x[1], root_.x[2]])
					...
					if np.linalg.norm(p1-p2) < 1e-3:
						...
						if abs(p1[0]-p2[1])>1e-3 or abs(p1[1]-p2[0])>1e-3:
							...
							# print(f"delta_mu_init = {mu_equations([pu[0], pl[0], pl[1]])}, delta_mu_fin = {mu_equations([root_.x[0], root_.x[1], root_.x[2]])}", flush=True)
	...
	def perform_secondary_binodal_blast(self, phi_positive, phi_negative, center, crit_point):
		...
		for idx, pu in enumerate(phi_positive):
			...
			for jdx, pl in enumerate(phi_negative):
				root_ = ROOT(mu_equations, [pu[1], pl[0], pl[1]], tol=1e-9, method='broyden1')
				if root_.success: # (np.abs(np.array(mu_equations(root_)))>1e-13).any():
					continue
				else:
					p1 = np.array([pu[0],   root_.x[0]])
					p2 = np.array([root_.x[1], root_.x[2]])
					...
					if np.linalg.norm(p1-p2) < 1e-3:
						...
						if abs(p1[0]-p2[1])>1e-3 or abs(p1[1]-p2[0])>1e-3:
							...
							# print(f"seed was phi_s = {pu[0]}, guess was phi_p = {pu[1]}, phi = {pl[0], pl[1]}", flush=True)
							# print(f"delta_mu_init = {mu_equations([pu[1], pl[0], pl[1]])}, delta_mu_fin = {mu_equations([root_.x[0], root_.x[1], root_.x[2]])}")
						break
				...
				if not root_.success: # (np.abs(np.array(mu_equations(root_)))>1e-13).any():
					...
					if np.linalg.norm(p1-p2) < 1e-3:
						...
						if abs(p1[0]-p2[1])>1e-3 or abs(p1[1]-p2[0])>1e-3:
							...
							# print(f"seed was phi_p = {pu[1]}, guess was phi_s = {pu[0]}, phi = {pl[0], pl[1]}", flush=True)
							# print(f"delta_mu_init = {mu_equations([pu[0], pl[0], pl[1]])}, delta_mu_fin = {mu_equations([root_.x[0], root_.x[1], root_.x[2]])}")
						break						
		return sol_bin_up, sol_bin_down
	######################################################
	...
	def build_bridges(self, sol_bin_pos, sol_bin_neg, center, crit_point, nadd):
		...
		for idx, pu in enumerate(sol_bin_pos_loaded[m_load+1:m_load+nadd-1]):
			...
			for jdx, pl in enumerate(sol_bin_neg_loaded[m_load+idx-nadd:m_load+1+idx+nadd]):
				root_ = root(mu_equations, [pu[0], pl[0], pl[1]], method='broyden1', tol=1e-9)
				if root_.success: # (np.abs(np.array(mu_equations(root_))) > 1e-6).any():
					continue
				else:
					p1 = np.array([root_.x[0], pu[1]])
					p2 = np.array([root_.x[1], root_.x[2]])
