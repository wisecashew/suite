...
class Binodal:
	...
	mu_c = lambda self, phi_s, phi_p: np.log(1-phi_s-phi_p) + 1 - (1-phi_s-phi_p) - self.vc/self.vs * phi_s - self.vc/self.vp * phi_p + self.vc * (phi_s**2 * self.chi_sc + phi_p**2 * self.chi_pc + phi_s * phi_p * (self.chi_sc + self.chi_pc - self.chi_ps) )

	delta_mu_s = lambda self, phi_s1, phi_p1, phi_s2, phi_p2: self.mu_s(phi_s1, phi_p1) - self.mu_s(phi_s2, phi_p2) 
	delta_mu_p = lambda self, phi_s1, phi_p1, phi_s2, phi_p2: self.mu_p(phi_s1, phi_p1) - self.mu_p(phi_s2, phi_p2)
	delta_mu_c = lambda self, phi_s1, phi_p1, phi_s2, phi_p2: self.mu_c(phi_s1, phi_p1) - self.mu_c(phi_s2, phi_p2)
	...
	d_delta_mu_s_dpp1 = lambda self, phi_s1, phi_p1, phi_s2, phi_p2: (self.chi_ps - self.chi_sc - self.chi_ps * phi_s1 + self.chi_pc * (-1 + phi_s1 + 2*phi_p1) + 1/self.vc - 1/self.vp) * self.vs
	d_delta_mu_s_dpp2 = lambda self, phi_s1, phi_p1, phi_s2, phi_p2: (self.chi_sc + self.chi_ps*(-1 + phi_s2) - self.chi_sc*phi_s2 - self.chi_pc * (-1 + phi_s2 + 2*phi_p2) - 1/self.vc + 1/self.vp) * self.vs

	d_delta_mu_p_dps1 = lambda self, phi_s1, phi_p1, phi_s2, phi_p2: self.vp*(self.chi_ps - self.chi_sc + 2*self.chi_sc*phi_s1 + self.chi_pc * (phi_p1 - 1) - self.chi_ps * phi_p1 + self.chi_sc * phi_p1 + 1/self.vc - 1/self.vs)
	d_delta_mu_p_dps2 = lambda self, phi_s1, phi_p1, phi_s2, phi_p2: self.vp*(self.chi_pc + self.chi_sc - 2*self.chi_sc*phi_s2 + self.chi_ps * (phi_p2 - 1) - self.chi_pc * phi_p2 - self.chi_sc * phi_p2 - 1/self.vc + 1/self.vs)
	...
	def delta_finder(self, phi1, phi2, delta_phip2):
		...
		D, Dx, Dy, Dz  = self.D_det_calcs (phi1, phi2, delta_phip2)
...
if __name__=="__main__":
	...
	for i in range(ncycles):
		...
		def dmu(phi_):
			...
			eq2 = B.delta_mu_p(phi_[0], phi_[1], phi_[2], phi_p2[-1]+delta_pp2[-1]) # /np.linalg.norm(np.array([phi_[0], phi_[1]]) - np.array([phi_[2], phi_p2[-1]+delta_pp2]))
			eq3 = B.delta_mu_c(phi_[0], phi_[1], phi_[2], phi_p2[-1]+delta_pp2[-1]) # /np.linalg.norm(np.array([phi_[0], phi_[1]]) - np.array([phi_[2], phi_p2[-1]+delta_pp2]))
			return [eq1, eq2, eq3]
