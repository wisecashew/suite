...
class Binodal:
			...
			for dp_p1 in pert:
				for dp_s2 in pert:
					for dp_p2 in pert_p2:
						def dmu_init(phi_):
							eq1 = B.delta_mu_s(phi_[0], phi_[1], phi_[2], crit_point[1]+dp_p2) 
							eq2 = B.delta_mu_p(phi_[0], phi_[1], phi_[2], crit_point[1]+dp_p2) 
							eq3 = B.delta_mu_c(phi_[0], phi_[1], phi_[2], crit_point[1]+dp_p2) 
							return [eq1, eq2, eq3]
	...
	def D_det_calcs_pert_s2(self, phi1, phi2, delta_phis2):
		...
		a3 = self.d_delta_mu_s_dpp2(phi1[0], phi1[1], phi2[0], phi2[1])
		a4 = -delta_phis2*self.d_delta_mu_s_dps2(phi1[0], phi1[1], phi2[0], phi2[1])
		

		b1 = self.d_delta_mu_p_dps1(phi1[0], phi1[1], phi2[0], phi2[1])
		b2 = self.d_delta_mu_p_dpp1(phi1[0], phi1[1], phi2[0], phi2[1])
		b3 = self.d_delta_mu_p_dpp2(phi1[0], phi1[1], phi2[0], phi2[1])
		b4 = -delta_phis2*self.d_delta_mu_p_dps2(phi1[0], phi1[1], phi2[0], phi2[1])
		

		c1 = self.d_delta_mu_c_dps1(phi1[0], phi1[1], phi2[0], phi2[1])
		c2 = self.d_delta_mu_c_dpp1(phi1[0], phi1[1], phi2[0], phi2[1])
		c3 = self.d_delta_mu_c_dpp2(phi1[0], phi1[1], phi2[0], phi2[1])
		c4 = -delta_phis2*self.d_delta_mu_c_dps2(phi1[0], phi1[1], phi2[0], phi2[1])
	...
	def delta_finder_pert_s2(self, phi1, phi2, delta_phis2):

		D, Dx, Dy, Dz = self.D_det_calcs_pert_s2(phi1, phi2, delta_phis2)
		delta_phis1   = Dx/D
		delta_phip1   = Dy/D
		delta_phip2   = Dz/D 

		return [delta_phis1, delta_phip1, delta_phip2]
		...
		for dp_s1 in pert:
			for dp_p1 in pert:
				for dp_p2 in pert:
					for dp_s2 in pert:
						def dmu_init(phi_):
							eq1 = B.delta_mu_s(phi_[0], phi_[1], crit_point[0]+dp_s2, phi_[2])
							eq2 = B.delta_mu_p(phi_[0], phi_[1], crit_point[0]+dp_s2, phi_[2])
							eq3 = B.delta_mu_c(phi_[0], phi_[1], crit_point[0]+dp_s2, phi_[2])
							return [eq1, eq2, eq3]
	...
	def run_cycles_pert_s2(self, ncycles, crit_point, continuation_b):
		...
		for i in range(ncycles):
			...
			if (np.abs(preprocessor) < 1e-16).all():
				delta_ps2.append(0.01)

			print(f"@ i = {i}/{ncycles}...", flush=True)

			phi1 = [phi_s1[-1], phi_p1[-1]]
			phi2 = [phi_s2[-1], phi_p2[-1]]
			deltas    = B.delta_finder_pert_s2(phi1, phi2, delta_ps2[-1])
			delta_ps1 = deltas[0]
			delta_pp1 = deltas[1]
			delta_pp2 = deltas[2]
			...
			print(f"Found root: phi1 = ({root[0], root[1]}), phi2 = ({phi_p2[-1]+delta_ps2[-1], root[2]})...", flush=True)
