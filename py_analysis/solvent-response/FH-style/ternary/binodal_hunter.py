...
if __name__=="__main__":
    ...
    mesh  = args.mesh
    phi_c = np.linspace (0.00001, 0.99999, mesh)
    phi_c = np.repeat (phi_c, mesh)
    phi_a = np.zeros  (phi_c.shape)
    for i in range (mesh):
        upper_lim = 0.99999 if phi_c[i*mesh] < 0.00001 else 1-phi_c[i*mesh] - 0.00001
        phi_a[i*mesh:(i+1)*mesh] = np.linspace (0.00001, upper_lim, mesh)

    to_keep = stab_crit (phi_a, 1-phi_a-phi_c, chi_ab, chi_bc, chi_ac) > 0
    phi_c   = phi_c [to_keep]
    phi_a   = phi_a [to_keep]

    print ("Calculating potentials...")
    chem_pot_a = mu_a (phi_a, 1-phi_a-phi_c)
    chem_pot_b = mu_b (phi_a, 1-phi_a-phi_c)
    chem_pot_c = mu_c (phi_a, 1-phi_a-phi_c)

    print ("Calculated potentials!")

    phis       = np.array([phi_a, 1-phi_c-phi_a, phi_c]).T
    ...
    col_indices = np.arange (len(phi_c))
    print ("Processed!")
