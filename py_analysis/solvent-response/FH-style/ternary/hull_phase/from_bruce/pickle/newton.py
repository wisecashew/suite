...
if __name__=="__main__":
	...
	for i in range(L_newton):
		if y_class_true[i] == 0:
			pass 
		elif y_class_true[i] == 1:
			ax.plot([y_comp_true[i,0], y_comp_true[i,3]], [1-y_comp_true[i,0]-y_comp_true[i,1], 1-y_comp_true[i,3]-y_comp_true[i,4]], [y_comp_true[i,1],y_comp_true[i,4]], c="gold", lw=0.5, alpha=0.5)
		elif y_class_true[i] == 2:
			ax.plot([y_comp_true[i,0], y_comp_true[i,3], y_comp_true[i,6]], [1-y_comp_true[i,0]-y_comp_true[i,1], 1-y_comp_true[i,3]-y_comp_true[i,4], 1-y_comp_true[i,6]-y_comp_true[i,7]], [y_comp_true[i,1],y_comp_true[i,4], y_comp_true[i,7]], c="skyblue", lw=0.5, alpha=0.5)
	...
	def triangle_mu(P):
		...
		return np.linalg.norm([eq1, eq2, eq3, eq4, eq5, eq6])

	# define the constraints
	tri_constraints = ({'type': 'ineq', 'fun': lambda x: 1 - x[0]},  # x[0] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[1]},  # x[1] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[0] - x[1]},  # x[2] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[2]},  # x[2] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[3]},  # x[2] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[2] - x[3]},  # x[2] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[4]},  # x[2] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[5]},  # x[2] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[4] - x[5]},  # x[2] <= 1
				{'type': 'ineq', 'fun': lambda x: x[0]},      # x[0] >= 0
				{'type': 'ineq', 'fun': lambda x: x[1]},      # x[1] >= 0
				{'type': 'ineq', 'fun': lambda x: x[2]},      # x[2] >= 0
				{'type': 'ineq', 'fun': lambda x: x[3]},      # x[0] >= 0
				{'type': 'ineq', 'fun': lambda x: x[4]},      # x[1] >= 0
				{'type': 'ineq', 'fun': lambda x: x[5]})      # x[2] >= 0

	line_constraints = ({'type': 'ineq', 'fun': lambda x: 1 - x[0]},  # x[0] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[1]},  # x[1] <= 1
				{'type': 'ineq', 'fun': lambda x: 1 - x[2]},  # x[2] <= 1
				{'type': 'ineq', 'fun': lambda x: x[0]},      # x[0] >= 0
				{'type': 'ineq', 'fun': lambda x: x[1]},      # x[1] >= 0
	...
	for i in range(L_newton):
		if y_class_pred[i] == 0:
			...
			def line_mu(P, phi_s):
				...
				return np.linalg.norm([eq1, eq2, eq3])
			ps1 = y_comp_pred[i][0]
			pp1 = y_comp_pred[i][1]
			ps2 = y_comp_pred[i][2]
			pp2 = y_comp_pred[i][3]
			print(f"ps1 + pp1 = {ps1+pp1}")
			print(f"ps2 + pp2 = {ps2+pp2}")
			root = minimize(line_mu, [pp1, ps2, pp2], args=(ps1), method="trust-constr", constraints=line_constraints)
			# print(f"line error = {line_mu(root, ps1)}", flush=True)
			p1 = np.array([ps1, root.x[0]])
			p2 = np.array([root.x[1], root.x[2]])
			if root.success: 
				ax.plot([p1[0], p2[0]], [1-p1[0]-p2[0],1-p1[1]-p2[1]], [p1[1], p2[1]], c="coral", markersize=0.5)
			def line_mu(P, phi_p):
				eq1 = M.delta_mu_s(P[0], phi_p, P[1], P[2])
				eq2 = M.delta_mu_p(P[0], phi_p, P[1], P[2])
				eq3 = M.delta_mu_c(P[0], phi_p, P[1], P[2])
				return np.linalg.norm([eq1, eq2, eq3])
			root = minimize(line_mu, [ps1, ps2, pp2], args=(pp1), method="Newton-CG", constraints=line_constraints)
			print(f"line root = {root}")
			# error = line_mu(root, pp1)
			p1 = np.array([root.x[0], pp1])
			p2 = np.array([root.x[1], root.x[2]])
			if root.success: 
				ax.plot([p1[0], p2[0]], [1-p1[0]-p2[0],1-p1[1]-p2[1]], [p1[1], p2[1]], c="coral", markersize=0.5)
			else:
			...
			print(f"ps1, pp1 = {ps1}, {pp1}")
			print(f"ps2, pp2 = {ps2}, {pp2}")
			print(f"ps3, pp3 = {ps3}, {pp3}")
			root = minimize(triangle_mu, [ps1, pp1, ps2, pp2, ps3, pp3], method="trust-constr", constraints=tri_constraints)
			print(f"triangle root = {root}", flush=True)
			p1 = np.array([root.x[0], root.x[1]])
			p2 = np.array([root.x[2], root.x[3]])
			p3 = np.array([root.x[4], root.x[5]])
			if root.success:
