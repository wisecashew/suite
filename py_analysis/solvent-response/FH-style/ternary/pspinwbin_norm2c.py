...
def binodal_plotter (dumpfile, chi_ab, chi_bc, chi_ac, va, vb, vc, crit, normal_to_crit):
    ...
    #         ax.plot    ([ref_bin[0][i,0],ref_bin[1][i,0]], [ref_bin[0][i,1],ref_bin[1][i,1]], lw=0.5, ls='--', markersize=0, zorder=10, c='skyblue', alpha=0.5)
...
def pbin_plotter (dumpfile, chi_ab, chi_bc, chi_ac, va, vb, vc, crit):
    ...
    ref_bin = binodal_plotter (dumpfile, chi_ab, chi_bc, chi_ac, va, vb, vc, crit, normal_to_crit)
...
if __name__=="__main__":
    ...
    if args.ternary:
        ...
        ax.scatter (crits[:,0], 1-crits[:,0]-crits[:,1], crits[:,1], color='k', edgecolors='steelblue', s=0.5)
        ax.scatter (p_a, p_c, p_b, s=0.01, color=cols, zorder=0, clip_on=True)
    ...
    pool    = mp.Pool(processes=len(crits))
    results = pool.starmap(pbin_plotter, zip(itertools.repeat(dumpfile), itertools.repeat(chi_ab), itertools.repeat(chi_bc), itertools.repeat(chi_ac), itertools.repeat(va), itertools.repeat(vb), itertools.repeat(vc), crits))
    pool.close()
    pool.join()
    ...
    for idx, res in enumerate(results):
        # this is the binodal
        if args.ternary:
            ax.scatter (res[0][:,0], 1-res[0][:,0]-res[0][:,1], res[0][:,1], c='silver',  s=0.125, zorder=11)
            ax.scatter (res[1][:,0], 1-res[1][:,0]-res[1][:,1], res[1][:,1], c='darkred', s=0.125, zorder=11)
        else:
            ax.scatter (res[0][:,0], res[0][:,1], c='silver',     s=0.125, zorder=11)
            ax.scatter (res[1][:,0], res[1][:,1], c='darkred',    s=0.125, zorder=11)

        if args.tl:
            for i in range (len(res[0])):
                if args.ternary:
                    ax.plot    ([res[0][i,0],res[1][i,0]], [1-res[0][i,0]-res[0][i,1],1-res[1][i,0]-res[1][i,1]], [res[0][i,1],res[1][i,1]], lw=0.5, ls='--', markersize=0, zorder=10, c='skyblue', alpha=0.01)
                else:
                    ax.plot    ([res[0][i,0],res[1][i,0]], [res[0][i,1],res[1][i,1]], lw=0.5, ls='--', markersize=0, zorder=10, c='skyblue', alpha=0.01)

        for i in range(len(res[0])):
            ff.write ("{:<2.6f} {:<2.6f} {:<2.6f} {:<2.6f} {:<2.6f} {:<2.6f}\n".format(res[0][i,0], res[0][i,1], 1-res[0][i,0]-res[0][i,1], res[1][i,0], res[1][i,1], 1-res[1][i,0]-res[1][i,0]))
