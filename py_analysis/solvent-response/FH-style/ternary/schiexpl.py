...
if __name__=="__main__":
    ...
    for i in range(0, chimeshsize, nchunk):
        for j in range (0, chimeshsize, nchunk):
            sector_chi_ab = chi_ab[i:i+nchunk, j:j+nchunk]
            sector_chi_bc = chi_bc[i:i+nchunk, j:j+nchunk]
            Z = get_colormesh (p_a, p_b, N, sector_chi_ab, sector_chi_bc, chi_ac[0])
            ax.pcolormesh     (sector_chi_ab, sector_chi_bc, Z, cmap=cm.ocean, norm=colors.Normalize(vmin=0, vmax=1), shading="auto")
            del Z
            del sector_chi_ab
            del sector_chi_bc
            count += 1
            print (f"Churned out sector {count}.")


    plt.savefig (f"cnscheck_vec_serial", dpi=1200)
