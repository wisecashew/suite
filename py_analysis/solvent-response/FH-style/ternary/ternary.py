...
def embelish(ax, tern_b):
	...
	if tern_b:
		...
		ax.taxis.set_ticks(positions)
		ax.laxis.set_ticks(positions)
		ax.raxis.set_ticks(positions)
...
def plot(ax, tern_b, edges_b, crits_b, crits, chi_ps, chi_pc, chi_sc, p_s, p_p, cols, root_up_p, root_lo_p, root_up_s, root_lo_s):
	...
	if tern_b:
		...
		if edges_b:
			meshsize            = 100
			phi_s               = np.linspace (0.001, 1-0.001, meshsize)

			r1 = root_up_s (p_s)
			r2 = root_lo_s (p_s)

			to_keep_1 = (~np.isnan(r1)) * (r1 <= 1) * (r1 >= 0) * (r1+p_s<=1)
			r1        = r1[to_keep_1]

			to_keep_2 = (~np.isnan(r2)) * (r2 <= 1) * (r2 >= 0) * (r2+p_s<=1)
			r2        = r2[to_keep_2]

			r3 = root_up_p (p_s)
			r4 = root_lo_p (p_s)

			to_keep_3 = (~np.isnan(r3)) * (r3 <= 1) * (r3 >= 0) * (r3+p_s<=1)
			r3        = r3[to_keep_3]

			to_keep_4 = (~np.isnan(r4)) * (r4 <= 1) * (r4 >= 0) * (r4+p_s<=1)
			r4        = r4[to_keep_4]


			# Plot the points
			ax.scatter (p_s[to_keep_1], 1-p_s[to_keep_1]-r1, r1, color='springgreen', s=1)
			ax.scatter (p_s[to_keep_2], 1-p_s[to_keep_2]-r2, r2, color='springgreen', s=1)

			# ax.scatter (r3, 1-p_s[to_keep_3]-r3, p_s[to_keep_3], color='darkturquoise', s=1)
			# ax.scatter (r4, 1-p_s[to_keep_4]-r4, p_s[to_keep_4], color='darkturquoise', s=1)

			spinodal_phi_s = np.hstack([p_s[to_keep_1], p_s[to_keep_2], r3, r4])
			spinodal_phi_p = np.hstack([r1, r2, p_s[to_keep_3], p_s[to_keep_4]])
		...
		if edges_b:
			...
			ax.scatter(p_s[to_keep_2], r2, color='springgreen',   s=1)
			# ax.scatter(r3, p_s[to_keep_3], color='darkturquoise', s=1)
			# ax.scatter(r4, p_s[to_keep_4], color='darkturquoise', s=1)
