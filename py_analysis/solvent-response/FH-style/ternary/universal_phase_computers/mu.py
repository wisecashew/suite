...
class Mu_PS:
	...
	def binodal_run(self, crit_point, continuation_b=False):
		if continuation_b:
			...
			phi_s1, phi_s2, phi_p1, phi_p2, delta_pp2 = self.find_solution_in_nbrhd(crit_point)
		...
		while not condition:
			...
			delta_ps1, delta_pp1, delta_ps2 = self.calc_perts(phi1, phi2, delta_pp2[-1]) # delta_finder_pert_p2(phi1, phi2, delta_pp2[-1])
...
class Mu_SC:
	...
	# calculate chemical potentials
	mu_s = lambda self, phi_s, phi_c: np.float64(np.log(phi_s)             + 1 - phi_s               - self.vs/self.vp * (1 - phi_s - phi_c) - self.vs/self.vc * phi_c + self.vs * ( (1 - phi_s - phi_c)**2 * self.chi_ps + phi_c**2 * self.chi_sc               + phi_s * (1 - phi_s - phi_c) * (self.chi_ps + self.chi_sc - self.chi_pc)))
	mu_p = lambda self, phi_s, phi_c: np.float64(np.log(1 - phi_s - phi_c) + 1 - (1 - phi_s - phi_c) - self.vp/self.vs * phi_s               - self.vp/self.vc * phi_c + self.vp * ( phi_s**2 * self.chi_ps               + phi_c**2 * self.chi_pc               + phi_s * phi_c               * (self.chi_ps + self.chi_pc - self.chi_sc)))
	mu_c = lambda self, phi_s, phi_c: np.float64(np.log(phi_c)             + 1 - phi_c               - self.vc/self.vp * (1 - phi_s - phi_c) - self.vc/self.vp * phi_s + self.vc * ( phi_s**2 * self.chi_sc               + (1 - phi_s - phi_c)**2 * self.chi_pc + (1 - phi_c - phi_p) * phi_p * (self.chi_sc + self.chi_pc - self.chi_ps)))

	delta_mu_s = lambda self, phi_s1, phi_c1, phi_s2, phi_c2: np.float64(self.mu_s(phi_s1, phi_c1) - self.mu_s(phi_s2, phi_c2))
	delta_mu_p = lambda self, phi_s1, phi_c1, phi_s2, phi_c2: np.float64(self.mu_p(phi_s1, phi_c1) - self.mu_p(phi_s2, phi_c2))
	delta_mu_c = lambda self, phi_s1, phi_c1, phi_s2, phi_c2: np.float64(self.mu_c(phi_s1, phi_c1) - self.mu_c(phi_s2, phi_c2))
	...
	d_delta_mu_s_dps2 = lambda self, phi_s1, phi_c1, phi_s2, phi_c2:  1 - 1/phi_s2 - self.vs/self.vp + self.chi_sc * (phi_s2 + (-1 + phi_cs + phi_s2)) * self.vs + self.chi_ps * ((1 - phi_c2 - phi_s2) + phi_s2)   * self.vs + self.chi_pc * ((1 - phi_c2 - phi_s2) - phi_s2)  * self.vs
	...
	d_delta_mu_c_dps2 = lambda self, phi_s1, phi_c1, phi_s2, phi_c2: self.chi_pc * ((1 - phi_c2 - phi_s2) + phi_s2) * self.vc + self.chi_pc * ((1 - phi_c2 - phi_s2)  + phi_s2)  * self.vc + self.chi_sc * (-phi_s2 + (-1 + phi_c2 + phi_s2)) * self.vc - self.vc/self.vp + self.vc/self.vs
		...
		for dp_s1 in pert:
			for dp_s2 in pert:
				for dp_c1 in pert:
					for dp_c2 in pert_c2:
						def dmu_init(phi_):
							eq1 = self.delta_mu_s(phi_[0], phi_[1], phi_[2], phi_c2[-1] + dp_c2)/np.linalg.norm(np.array([phi_[0], phi_[1]]) - np.array([phi_[2], crit_point[1]+dp_p2]))
							eq2 = self.delta_mu_p(phi_[0], phi_[1], phi_[2], phi_c2[-1] + dp_c2)/np.linalg.norm(np.array([phi_[0], phi_[1]]) - np.array([phi_[2], crit_point[1]+dp_p2]))
							eq3 = self.delta_mu_c(phi_[0], phi_[1], phi_[2], phi_c2[-1] + dp_c2)/np.linalg.norm(np.array([phi_[0], phi_[1]]) - np.array([phi_[2], crit_point[1]+dp_p2]))
							return [eq1, eq2, eq3]
						...
						if (np.abs(dmu_init(root))>1e-12).any():
							...
							phi_c1.append(root[1])
							phi_s2.append(root[2])
							phi_c2.append(crit_point[1]+dp_c2)
							break
	...
	def binodal_run(self, crit_point, continuation_b=False):
		...
		while not condition:
			...
			if (np.abs(dmu(root))>1e-12).any():
				print(f"Bad root: phi1 = ({root[0], root[1]}), phi2 = ({root[2], phi_c2[-1]+delta_pc2[-1]})...", flush=True)
				delta_pp2.append(delta_pc2[-1]/2)
				continue
			elif np.linalg.norm(p1-p2) < 1e-3: 
				print(f"Too close: phi1 = ({root[0], root[1]}), phi2 = ({root[2], phi_c2[-1]+delta_pc2[-1]})...", flush=True)
				delta_pp2.append(delta_pc2[-1]/2)
