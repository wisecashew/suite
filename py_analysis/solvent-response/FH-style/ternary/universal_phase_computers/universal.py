...
if __name__=="__main__":
	...
	for idx_tup in list(BINODALS["groupings"].keys()):
		...
		if BINODALS["groupings"][idx_tup]["identity"] == "unity":
			...
			uphi           = phi[:, 0:2]
			adj_uphi       = (uphi-(cp_center[0:2]+0.05*nv_center))/np.linalg.norm(uphi-(cp_center[0:2]+0.05*nv_center), axis=1).reshape(-1,1)
			...
			for probe in to_probe:
				...
				if probe == "positive":
					print(f"\tInside the boolean", flush=True)
			...
			ax.scatter(B[0][:,0], 1-B[0][:,0]-B[0][:,1], B[0][:,1], c='white', s=0.5)
			ax.scatter(B[1][:,0], 1-B[1][:,0]-B[1][:,1], B[1][:,1], c='black', s=0.5)
	...
	if len(stable_islands)>1:
		...
		for i in range(len(keys)):
			...
			# BINODALS["island_scans"][keys[i]][keys[i][0]]["intersections"] = dict()
			# BINODALS["island_scans"][keys[i]][keys[i][1]]["intersections"] = dict()
		...
		for key in BINODALS["island_scans"]:
			...
			for k2 in BINODALS["island_scans"][key]["intersections"]:
				if BINODALS["island_scans"][key]["intersections"][k2]["boolean"]:
					triangle.append(k2)
